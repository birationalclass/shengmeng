...
function waveModel(wave){
  const early=Balance.earlyWaveProfile(wave);
  const count=Math.max(7,Math.floor((9+wave*2.55+Math.pow(wave,1.08))*early.countScale));
  const hp=36*Math.pow(1.15,wave-1)*(1+Math.max(0,wave-20)*.018)*early.hpScale*.88;
  const midBosses=Math.min(3,1+Math.floor(Math.max(0,wave-5)/8));
  const doubleBoss=wave>=7&&wave%7===0;
  const colossal=wave%3===0;
  const effectiveHp=count*hp*.98+midBosses*hp*5.6+hp*8.8*(colossal?1.35:1)*(doubleBoss?2:1);
  const attack=Math.pow(1.07,wave-1)*(1+Math.floor((wave-1)/5)*.07)*early.attackScale;
...
const checkpoints=[1,3,5,10,15,20,25,30].map((wave)=>{const values=runs.map((run)=>run[wave-1].clearSeconds).sort((a,b)=>a-b);const sample=runs[0][wave-1];return {wave,enemies:sample.count,hp:sample.hp,attack:sample.attack,p25:values[49],median:values[99],p75:values[149]};});
