...
function waveModel(wave){
  const early=Balance.earlyWaveProfile(wave),late=Balance.lateWaveProfile(wave),pressure=Balance.waveEnemyPressureMultiplier(wave);
  const count=Math.max(20+wave*2,Math.floor((20+wave*3.4+Math.pow(wave,1.1)*1.25)*early.countScale));
  const baseHp=36*Math.pow(1.15,wave-1)*(1+Math.max(0,wave-20)*.018)*early.hpScale*(wave<=5?.7:1)*late.hpScale*pressure.hp,hp=baseHp*(wave<=10?.8:1);
  ...
  const attack=Math.pow(1.07,wave-1)*(1+Math.floor((wave-1)/5)*.07)*early.attackScale*late.attackScale*pressure.attack;
...
function simulate(seed=0x61ac71){
  ...
  const openingProbabilities=[...Balance.BASE_STAR_PROBABILITIES.slice(0,4),0,0],openingTotal=openingProbabilities.reduce((sum,value)=>sum+value,0);for(let opening=0;opening<10;opening++){const star=Balance.rollStar(openingProbabilities.map((value)=>value/openingTotal),rng);draws+=1;power*=1+[.035,.065,.105,.16,.24,.38][star-1];if(star>=4)skillUptime+=.012*star;if(activeTowers<3&&rng()<.22)activeTowers+=1;}
  const rows=[];
...
assert(checkpoints[0].median<75,"第一波在扩容后仍不会因基础炮塔降档而拖成长局");
