...
assert.match(releaseId, /-v152-tiered-achievements-replay-library-r3$/, "当前发布版本为 v152 分级成就、成长复演与折叠图鉴版本");
...
assert(indexSource.includes('id="achievementHudButton"')&&indexSource.includes('id="relicHudButton"')&&/\$\("#achievementHudButton"\)\.addEventListener\("click",\(\)=>openPanel\("achievementOverlay",renderAchievements\)\)/.test(gameSource)&&/\$\("#relicHudButton"\)\.addEventListener\("click",\(\)=>openPanel\("relicOverlay",renderRelicArchive\)\)/.test(gameSource),"短屏桌面也可从左上固定操作区打开成就档案与遗物库");
...
assert(gameSource.includes('renderCard=(card,level)=>')&&gameSource.includes('<span class="card-top"><b>${stars}</b>')&&/\.library-card \.glyph \{/.test(stylesSource), "卡牌图鉴保留独立等级卡面、星带、主图标和玻璃卡体结构");
...
for(const relic of ["vanguard_chart","supply_capsule","barrier_seed","time_prism","healing_bastion_core","rangefinder_array","arsenal_core","tempo_core","swarm_war_archive","bounty_echo_beacon","barrier_origin_core"])assert(gameSource.includes(`id:"${relic}"`), `遗物 ${relic} 已实现`);
for(const relic of ["bullet_replay_core","laser_replay_core","missile_replay_core","frost_replay_core","arc_replay_core","support_replay_core"])assert(gameSource.includes(`relic:"${relic}"`), `遗物 ${relic} 已实现`);
...
assert(/const progress=ensureProgress\(account\);evaluateAchievements\(false\);selectHighestUnlockedDifficulty\(progress\);state\.best/.test(gameSource),"旧账号登录时补发遗物成就并自动选择已解锁最高难度");
...
const barrierSegmentThresholds=Array.from({length:10},(_,index)=>1e9*Math.pow(10,index));
const barrierOriginBonuses=[0,...Array.from({length:10},(_,index)=>10*Math.pow(10,index))];
assert.deepEqual(barrierSegmentThresholds,[1e9,1e10,1e11,1e12,1e13,1e14,1e15,1e16,1e17,1e18],"微壁垒纪元按任一分段 1b 至 1e18 共十级成长");
assert.deepEqual(barrierOriginBonuses,[0,10,100,1000,10000,100000,1000000,10000000,100000000,1000000000,10000000000],"壁垒原核按等级为五段初始总量固定增加 10 至 10b");
assert(/const BARRIER_SEGMENT_RELIC_THRESHOLDS = Array\.from\(\{length:GENERAL_RELIC_LEVEL_CAP\},\(_,index\)=>1e9\*Math\.pow\(10,index\)\)/.test(gameSource)&&/const BARRIER_ORIGIN_TOTAL_BONUSES = \[0,\.\.\.Array\.from\(\{length:GENERAL_RELIC_LEVEL_CAP\},\(_,index\)=>10\*Math\.pow\(10,index\)\)\]/.test(gameSource),"代码中的单段屏障门槛与初始总量奖励均为十级十倍进位");
assert(/segmentMaximum=Math\.max\(0,\.\.\.state\.barrierSegments\.map\(\(segment\)=>Math\.max\(0,Number\(segment\.maxHp\)\|\|0\)\)\)/.test(gameSource)&&/maxBarrierSegmentMax/.test(gameSource),"微壁垒纪元读取任一小屏障历史最高生命上限而非五段总量");
assert(/startingBarrier=INITIAL_BARRIER_TOTAL\*\(1\+\(runRelicLevels\.barrier_seed\|\|0\)\*\.1\)\+\(BARRIER_ORIGIN_TOTAL_BONUSES\[runRelicLevels\.barrier_origin_core\|\|0\]\|\|0\)/.test(gameSource),"新局先结算屏障晶种百分比，再精确相加壁垒原核的五段总量奖励");
assert.equal(500*(1+10*.1)+barrierOriginBonuses[10],10000001000,"满级屏障晶种与壁垒原核的新局五段总生命为 10,000,001,000");
...
const replayDefBlock=gameSource.match(/const TURRET_REPLAY_DEFS=\[[\s\S]*?\n  \];/)?.[0]||"";
assert.equal((replayDefBlock.match(/turretId:/g)||[]).length,6,"六类炮塔各有一个终结者成就与复演遗物");
for(const id of ["bullet","laser","missile","frost","arc","support"])assert(new RegExp(`turretId:"${id}"[^\\n]+relic:"${id}_replay_core"`).test(replayDefBlock),`${id} 终结者绑定独立复演核心`);
assert(/function growthReplayChance\(turretId\)[^\n]+\(10\+level\)\/100[^\n]+\.6/.test(gameSource),"成长复演概率严格从 LV.1 的 11% 逐级增长至 LV.50 的 60%");
assert(/function replayGrowthBonus\(turretId,gain\)[^\n]+Math\.random\(\)<chance\?gain:0/.test(gameSource),"每次自然成长成功后至多追加一次同额复演且不递归");
assert(/growSupportHealingFromKill\(enemy\)[\s\S]{0,420}const replay=replayGrowthBonus\("support",gain\),totalGain=gain\+replay/.test(gameSource)&&/growLeadingTurretAttackFromKill\(\)[\s\S]{0,520}const replay=replayGrowthBonus\(leader\.turretId,gain\),totalGain=gain\+replay/.test(gameSource),"五类攻击成长与支援治疗成长均在自然增益成功后接入终结者复演");
const damageTotalsSource=gameSource.match(/  function runTurretDamageTotals\([^\r\n]+/)?.[0];
const terminatorVictorySource=gameSource.match(/  function recordTurretTerminatorVictory\([^\r\n]+/)?.[0];
assert(damageTotalsSource&&terminatorVictorySource,"可提取整局炮塔伤害冠军结算函数");
const terminatorSandbox={state:{stats:{damage:{"子弹炮":100,"激光炮":100,"支援炮塔":50,"分段屏障":99999,"相位反冲":88888}}},TURRET_REPLAY_DEFS:[
  {turretId:"bullet",name:"子弹炮",relic:"bullet_replay_core"},{turretId:"laser",name:"激光炮",relic:"laser_replay_core"},{turretId:"missile",name:"导弹炮",relic:"missile_replay_core"},{turretId:"frost",name:"冰霜炮",relic:"frost_replay_core"},{turretId:"arc",name:"电弧炮",relic:"arc_replay_core"},{turretId:"support",name:"支援炮塔",relic:"support_replay_core"}
],meterTurretGroup:(source)=>({"子弹炮":"bullet","激光炮":"laser","导弹炮":"missile","冰霜炮":"frost","电弧炮":"arc","支援炮塔":"support","分段屏障":"support","相位反冲":"support"})[source]||null,relicLevel:(id,progress)=>progress.relicLevels[id]||0,clamp:(value,min,max)=>Math.max(min,Math.min(max,value)),showToast:()=>{},Date,Math,globalThis:null};
terminatorSandbox.globalThis=terminatorSandbox;
vm.runInNewContext(`${damageTotalsSource}\n${terminatorVictorySource}\nglobalThis.__record=recordTurretTerminatorVictory;globalThis.__totals=runTurretDamageTotals;`,terminatorSandbox);
const terminatorProgress={unlockedRelics:[],relicLevels:{},achievements:{}};
assert.equal(terminatorSandbox.__totals().support,50,"分段屏障与全局相位反冲不会伪装成支援炮塔冠军伤害");
const tieWinners=terminatorSandbox.__record(terminatorProgress,{rating:17});
assert.deepEqual(Array.from(tieWinners,entry=>entry.turretId),["bullet","laser"],"整局并列伤害第一的炮塔会共同完成终结者成就");
assert.equal(terminatorProgress.relicLevels.bullet_replay_core,18,"黑洞 III 对应连续难度 LV.18");
assert.equal(terminatorProgress.relicLevels.laser_replay_core,18,"并列冠军获得相同难度等级");
terminatorSandbox.state.stats.damage={"子弹炮":200};terminatorSandbox.__record(terminatorProgress,{rating:2});
assert.equal(terminatorProgress.relicLevels.bullet_replay_core,18,"重打较低难度不会降低终结者遗物等级");
terminatorSandbox.state.stats.damage={"支援炮塔":300,"分段屏障":99999};terminatorSandbox.__record(terminatorProgress,{rating:49});
assert.equal(terminatorProgress.relicLevels.support_replay_core,50,"真实支援归属伤害可在终焉奇点 V 把纳米复演核心升至 LV.50");
assert(/function renderAchievements\(\)\{const progress=ensureProgress\(\);\$\("#achievementList"\)\.innerHTML=achievementTrackCards\(progress\)\.join\(""\);\}/.test(gameSource)&&!/function renderAchievements\(\)[^\n]+ACHIEVEMENTS\.map/.test(gameSource),"同类别成就只渲染一个等级轨道，不再展开 77 个旧子项或 300 个终结者等级");
...
assert(/state\.supportHealGrowth=Math\.max\(0,state\.supportHealGrowth\|\|0\)\+totalGain/.test(gameSource)&&!gameSource.includes("current>=softCap?.25:1"),"每次治疗成长触发均完整累加自然成长与至多一次终结者复演");
...
assert(/function libraryDisplayLevels\(card,rank=cardRank\(card\.id\),designMode=isDesignMode\(\)\)\{if\(designMode\)return Array\.from\(\{length:card\.max\},\(_,index\)=>index\+1\);return \[clamp\(Math\.floor\(Number\(rank\)\|\|1\),1,card\.max\)\];\}/.test(gameSource),"普通图鉴只显示当前最高已获得等级，未获得显示 I；设计账号仍可逐级评审");
const displayLevelsSource=gameSource.match(/ function libraryDisplayLevels\([^\r\n]+/)?.[0];
const displayLevelsSandbox={clamp:(value,min,max)=>Math.max(min,Math.min(max,value)),cardRank:()=>0,isDesignMode:()=>false,globalThis:null};displayLevelsSandbox.globalThis=displayLevelsSandbox;
vm.runInNewContext(`${displayLevelsSource};globalThis.__levels=libraryDisplayLevels;`,displayLevelsSandbox);
assert.deepEqual(Array.from(displayLevelsSandbox.__levels({max:5,id:"sample"},3,false)),[3],"普通账号拥有 III 时图鉴只展示 III");
assert.deepEqual(Array.from(displayLevelsSandbox.__levels({max:5,id:"sample"},0,false)),[1],"普通账号未拥有卡牌时图鉴只展示 I 作为预览");
assert.deepEqual(Array.from(displayLevelsSandbox.__levels({max:5,id:"sample"},3,true)),[1,2,3,4,5],"设计账号继续显示所有等级以便逐级评审");
const collectibleCards=cards.filter((card)=>!card.designRemoved),collectibleRankTotal=(tag)=>collectibleCards.filter((card)=>card.tags.includes(tag)).reduce((sum,card)=>sum+card.max,0);
assert.equal(collectibleRankTotal("弹道"),93,"子弹炮图鉴满收集显示 93/93，不再把 6 级废卡计入分母");
assert.equal(collectibleRankTotal("支援"),108,"支援图鉴满收集显示 108/108，不再被依赖废卡污染");
assert.equal(collectibleRankTotal("特殊"),129,"特殊图鉴满收集显示 129/129，不再把 47 级废卡计入分母");
assert(/function libraryCardsInCategory\(category\)\{return collectibleCards\(\)\.filter/.test(gameSource)&&/CARD_LIBRARY_CATEGORIES\.map\(\(item\)=>\{const cards=libraryCardsInCategory\(item\)/.test(gameSource),"分类按钮与图鉴网格统一使用可获取卡池统计");
assert(/function normalizeSavedCardEntries\(entries\)[^\n]+LEGACY_CARD_ID_ALIASES[^\n]+card\.designRemoved/.test(gameSource)&&/function normalizeRunCardEntries\(run=\{\}\)[^\n]+normalizeSavedCardEntries\(run\.cards\)[^\n]+\["rate","bullet_base_rate"\]/.test(gameSource)&&/resumeCards"\)\.textContent=normalizeRunCardEntries\(run\)/.test(gameSource),"旧存档续局卡数与实际加载共用废卡过滤、别名及旧商店迁移结果");
const normalizeSavedSource=gameSource.match(/  function normalizeSavedCardEntries\([^\r\n]+/)?.[0],normalizeRunSource=gameSource.match(/  function normalizeRunCardEntries\([^\r\n]+/)?.[0];
assert(normalizeSavedSource&&normalizeRunSource,"可提取统一旧卡迁移函数");
const migrationSandbox={CARD_MAP:{bullet_base_rate:{max:5},bullet_base_crit:{max:5},bullet_base_crit_power:{max:5},frost_trace_spread_2:{max:5},removed:{max:5,designRemoved:true}},LEGACY_CARD_ID_ALIASES:{evo_frost_2_1:"frost_trace_spread_2"},LEGACY_FIREPOWER_CARD_IDS:new Set(["global_damage"]),clamp:(value,min,max)=>Math.max(min,Math.min(max,value)),Math,globalThis:null};migrationSandbox.globalThis=migrationSandbox;
vm.runInNewContext(`${normalizeSavedSource}\n${normalizeRunSource}\nglobalThis.__normalizeSaved=normalizeSavedCardEntries;globalThis.__normalizeRun=normalizeRunCardEntries;`,migrationSandbox);
const mixedLegacyCards=new Map(migrationSandbox.__normalizeRun({cards:[["bullet_base_rate",5]],shop:{rate:1}}));
assert.equal(mixedLegacyCards.get("bullet_base_rate"),5,"旧 shop.rate 只按最高级合并，不会把当前 V 级攻速降回 I 级");
const pureLegacyCards=new Map(migrationSandbox.__normalizeRun({cards:[],shop:{rate:2,crit:3,critDamage:4}}));
assert.equal([...pureLegacyCards.values()].reduce((sum,rank)=>sum+rank,0),9,"纯旧商店存档在续局页与实际加载时都统计三项迁移等级");
const aliasedLegacyCards=new Map(migrationSandbox.__normalizeRun({cards:[["evo_frost_2_1",3],["frost_trace_spread_2",2],["removed",5]]}));
assert.deepEqual([...aliasedLegacyCards.entries()],[["frost_trace_spread_2",3]],"旧冰霜别名与新 ID 取最高等级且废卡被丢弃");
assert(/cardChoiceHistory:\(run\.cardChoiceHistory\|\|\[\]\)[^\n]+LEGACY_CARD_ID_ALIASES\[rawId\][^\n]+!card\.designRemoved/.test(gameSource),"旧选牌历史同样迁移别名并过滤废卡，避免影响加载后的抽卡权重");
const highestMajorSource=gameSource.match(/  function unlockedDifficultyMajorIndex\([^\r\n]+/)?.[0],highestMinorSource=gameSource.match(/  function unlockedDifficultyMinor\([^\r\n]+/)?.[0],highestDifficultySource=gameSource.match(/  function highestUnlockedDifficulty\([^\r\n]+/)?.[0];
const difficultySandbox={DIFFICULTY_MAJORS:["cruise","deep","singularity","terminus","nebula","cluster","galaxy","quasar","cosmic_web","finality"].map((id)=>({id})),ensureProgress:()=>({difficultyClears:{}}),Math,globalThis:null};difficultySandbox.globalThis=difficultySandbox;
vm.runInNewContext(`${highestMajorSource}\n${highestMinorSource}\n${highestDifficultySource}\nglobalThis.__highest=highestUnlockedDifficulty;`,difficultySandbox);
assert.deepEqual({...difficultySandbox.__highest({difficultyClears:{}})},{major:"cruise",minor:1,rating:0},"新账号初始登录自动选择行星 I");
assert.deepEqual({...difficultySandbox.__highest({difficultyClears:{cruise:5}})},{major:"deep",minor:1,rating:5},"行星 V 已通关账号初始登录自动选择恒星 I");
const allDifficultyClears=Object.fromEntries(difficultySandbox.DIFFICULTY_MAJORS.map((item)=>[item.id,5]));
assert.deepEqual({...difficultySandbox.__highest({difficultyClears:allDifficultyClears})},{major:"finality",minor:5,rating:49},"全解锁账号初始登录自动选择终焉奇点 V");
assert(/function refreshDifficultyOptions\(\)[^\n]+if\(!state\.authenticated\)/.test(gameSource),"未登录初始化不会把已保存的难度设置夹回行星 I");
assert(indexSource.includes('id="menuButton"')&&indexSource.includes('id="menuConfirmOverlay"')&&indexSource.includes('id="confirmMenuButton"'),"左上角提供保存并返回初始菜单按钮与二次确认层");
assert(/function confirmReturnToMenu\(\)\{if\(!saveRun\(false\)\)\{showToast\("保存失败，仍停留在当前战局"\);return;\}[^\n]+showStartScreen\(\)/.test(gameSource),"返回初始菜单必须先保存成功，失败时不会离开当前战局");
assert(/function cancelReturnToMenu\(\)[^\n]+state\.paused=state\.returnMenuWasPaused/.test(gameSource),"取消返回菜单会恢复打开确认层前的暂停状态");
