...
for(const [turret,family] of Object.entries(families)){
  ...
  for(const stat of turret==="support"?["range","base_rate","base_heal","base_crit","base_crit_power"]:["range","base_rate","base_damage","base_crit","base_crit_power"]){
...
for(const id of ["cryo_missile_echo","laser_arc_echo","bullet_support_echo"]){const card=cards.find((item)=>item.id===id);assert.equal(card.star,6,`${id} 为六星连携`);}
