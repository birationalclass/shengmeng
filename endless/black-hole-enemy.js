(() => {
    ...
    void main(){
      ...
      for(int q=0;q<16;q++){
        if(foregroundTransmittance>.012){
          float travel=mix(5.2,14.75,(float(q)+.5)/16.);vec3 directHit=cameraPos+ray*travel;float directR=length(directHit.xz);
      ...
      for(int k=0;k<4;k++){
        for(int s=0;s<19;s++){
          float slice=(float(s)-9.)*.034,phi=basePhi+float(k)*PI+slice;
  ...
  const compositeSource = `#version 300 es
    ...
    void main(){vec4 scene=texture(uScene,vUv);vec3 bloom=texture(uBloom,vUv).rgb;vec3 c=scene.rgb+bloom*uBloomGain;c*=vec3(1.02,1.,.97);c=1.-exp(-c*1.42);c=pow(max(c,vec3(0.)),vec3(.4545));vec2 edgeP=abs((vUv-.5)*2.);float edgeFade=(1.-smoothstep(.78,1.,edgeP.x))*(1.-smoothstep(.78,1.,edgeP.y));float alpha=max(scene.a,smoothstep(.035,.14,max(max(c.r,c.g),c.b)))*edgeFade;outColor=vec4(c*edgeFade,alpha);}
  `;
  ...
  class SharedBlackHoleRenderer {
    ...
    render(enemy, elapsed) {
      ...
      const frame=Math.floor(elapsed*12),palette=PALETTES[enemy.blackHolePalette]||PALETTES.violet,key=`${enemy.id}:${enemy.blackHolePalette}:${frame}:${enemy.hit>0?1:0}`;
  ...
  function drawExplosion(ctx, particle) {
    ...
    const ease=1-Math.pow(1-progress,3),fade=Math.pow(1-progress,2.25),extent=radius*2.72;
