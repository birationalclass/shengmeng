(() => {
  ...
  class SharedBlackHoleRenderer {
    ...
    render(enemy, elapsed) {
      ...
      const frame=Math.floor(elapsed*this.targetFps),palette=PALETTES[enemy.blackHolePalette]||PALETTES.violet,key=`${enemy.blackHolePalette}:${enemy.visualYaw}:${enemy.visualPitch}:${enemy.visualRoll}:${frame}`;
  ...
  // The boss is drawn at roughly 120–180 CSS pixels in the game. Rendering a
  // 320 px HDR source every animation step only increases GPU/CPU readback
  // cost, not visible detail. A compact source is cached in memory and blended
  // between frames on the cheap 2D battle canvas.
  const rendererSize = lowPowerDevice ? 192 : 224;
  const renderer = new SharedBlackHoleRenderer(rendererSize);
  const CACHE_FPS = lowPowerDevice ? 4 : 5;
  const CACHE_FRAMES = lowPowerDevice ? 4 : 5;
  const MAX_CACHE_VARIANTS = 3;
  renderer.targetFps = CACHE_FPS;
  const animationCache = new Map();

  function quantize(value,step){return Math.round((Number(value)||0)/step)*step;}
  function cacheKey(enemy){return `${enemy.blackHolePalette||"violet"}:${quantize(enemy.visualYaw,14)}:${quantize(enemy.visualPitch||14,6)}:${quantize(enemy.visualRoll,10)}`;}
  function cachedEnemy(enemy){return {...enemy,hit:0,visualYaw:quantize(enemy.visualYaw,14),visualPitch:quantize(enemy.visualPitch||14,6),visualRoll:quantize(enemy.visualRoll,10)};}
  function copyRendererFrame(){const frame=document.createElement("canvas");frame.width=frame.height=rendererSize;frame.getContext("2d",{alpha:true}).drawImage(renderer.canvas,0,0);return frame;}
  function trimCache(){while(animationCache.size>MAX_CACHE_VARIANTS){const oldest=animationCache.keys().next().value,entry=animationCache.get(oldest);entry.disposed=true;for(const frame of entry.frames)if(frame){frame.width=0;frame.height=0;}animationCache.delete(oldest);}}
  function renderFrame(entry,index){if(entry.frames[index])return entry.frames[index];const sampleTime=index/CACHE_FPS;if(!renderer.render(entry.enemy,sampleTime))return null;const frame=copyRendererFrame();entry.frames[index]=frame;return frame;}
  function warmCache(entry,index=1){while(index<CACHE_FRAMES&&entry.frames[index])index+=1;if(entry.disposed||index>=CACHE_FRAMES)return;const run=(deadline)=>{if(entry.disposed)return;if(deadline&&!deadline.didTimeout&&deadline.timeRemaining()<7){warmCache(entry,index);return;}renderFrame(entry,index);setTimeout(()=>warmCache(entry,index+1),180);};if("requestIdleCallback" in window)requestIdleCallback(run,{timeout:3200});else setTimeout(()=>run(null),220);}
  function animationFor(enemy){const key=cacheKey(enemy);let entry=animationCache.get(key);if(entry){animationCache.delete(key);animationCache.set(key,entry);return entry;}entry={enemy:cachedEnemy(enemy),frames:Array(CACHE_FRAMES).fill(null),disposed:false};animationCache.set(key,entry);trimCache();renderFrame(entry,0);warmCache(entry);return entry;}

  function nearestCachedFrame(entry,index,direction=1){for(let offset=0;offset<CACHE_FRAMES;offset+=1){const candidate=(index+offset*direction+CACHE_FRAMES*2)%CACHE_FRAMES;if(entry.frames[candidate])return entry.frames[candidate];}return null;}
  ...
  function draw(ctx, enemy, radius, elapsed) {
    const entry=animationFor(enemy),framePosition=(elapsed*CACHE_FPS)%CACHE_FRAMES,frameIndex=Math.floor(framePosition),nextIndex=(frameIndex+1)%CACHE_FRAMES,
      frame=entry.frames[frameIndex]||nearestCachedFrame(entry,frameIndex,-1),next=entry.frames[nextIndex],mix=framePosition-frameIndex;
    if (!frame) return false;
    const extent = visualExtent(radius);
    ctx.save();ctx.globalAlpha=1;if(enemy.hit>0){ctx.filter=`brightness(${1+Math.min(.55,enemy.hit*3.4)}) saturate(1.08)`;ctx.shadowColor="#ffffff";ctx.shadowBlur=8;}ctx.drawImage(frame,-extent,-extent,extent*2,extent*2);if(next&&next!==frame&&mix>.02){ctx.globalAlpha=mix;ctx.drawImage(next,-extent,-extent,extent*2,extent*2);}ctx.restore();
    return true;
  }
