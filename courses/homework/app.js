...
const bookStep=()=>settings.reading==='double'&&matchMedia('(min-width:1001px)').matches?2:1;
...
function captureLeaf(leaf){
 ...
 Object.assign(copy.style,{position:'absolute',left:rect.left+'px',top:rect.top+'px',width:rect.width+'px',height:rect.height+'px',margin:'0',transform:'none',zoom:'1',pointerEvents:'none'});
 copy.inert=true;copy.setAttribute('aria-hidden','true');return copy;
}
...
function bind(){
 ...
 $$('[data-turn]').forEach(b=>b.onclick=()=>turnBook(Number(b.dataset.turn)));
...
window.addEventListener('resize',()=>{cancelPageTurn();syncBookView();sizeNotes();syncSettingsMask()});
