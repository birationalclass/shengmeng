...
(()=>{
  ...
  function renderQuotient(){
    ...
    function draw(){
      ...
      values.forEach((v,i)=>{
        ...
        content+=`<g class="group-point" data-integer="${v}" tabindex="0" role="button" aria-label="选择代表元 ${v}，属于 [${cl}] 类" opacity="${1-collapse*.75}"><circle cx="${x}" cy="${y}" r="${v===a?16:13}" fill="${v===a?colors[cl]:soft[cl]}" stroke="${colors[cl]}" stroke-width="${v===a?2:1}"/><text x="${x}" y="${y+5}" font-size="${compact?15:12}" text-anchor="middle" fill="${v===a?'var(--diagram-solid-text,#fff)':colors[cl]}" opacity="${1-collapse}">${v}</text></g>`;
      });
  ...
  function renderSymmetry(){
    ...
    function triangle(points,cx){const cy=133;let s=`<path d="${base.map(([x,y],i)=>`${i?'L':'M'}${x+cx},${y+cy}`).join('')}Z" fill="none" stroke="var(--diagram-line,#b9c9bd)" stroke-dasharray="5 6" stroke-width="1.3"/><path d="${points.map(([x,y],i)=>`${i?'L':'M'}${x+cx},${y+cy}`).join('')}Z" fill="var(--diagram-wash,#dcebe255)" stroke="var(--diagram-edge,#547f6d)" stroke-width="1.6"/>`;
      points.forEach(([x,y],i)=>{s+=`<circle cx="${cx+x}" cy="${cy+y}" r="19" fill="${colors[i]}"/><text x="${cx+x}" y="${cy+y+6}" text-anchor="middle" font-size="19" fill="var(--diagram-solid-text,#fff)">${i+1}</text>`;});return s;}
    function draw(left=positions(M.d3mul(B,A)),right=positions(M.d3mul(A,B))){$('symmetry-graph').innerHTML=svg(`<path d="M320 35V240" stroke="var(--diagram-line,#e1e6dc)"/>${triangle(left,160)}${triangle(right,480)}<text x="160" y="261" text-anchor="middle" font-size="17" fill="var(--diagram-muted,#61736c)">先 A，后 B</text><text x="480" y="261" text-anchor="middle" font-size="17" fill="var(--diagram-muted,#61736c)">先 B，后 A</text>`,'0 0 640 285','左右三角形按两种顺序应用变换，编号与颜色共同追踪三个顶点');}
  ...
  function renderOrder(){
    ...
    function draw(progress=0){
      ...
      let s='';
      if(key==='integer'){
        const unit=32,center=380,limit=compact?4:9;for(let j=-limit;j<=limit;j++)s+=`<path d="M${center+j*unit} 139v12" stroke="var(--diagram-ghost,#a2b4a5)"/><text x="${center+j*unit}" y="174" text-anchor="middle" font-size="13" fill="var(--diagram-muted,#64766c)">${j}</text>`;
        ...
        E.forEach((x,i)=>{const [cx,cy]=P[i],visited=seq.slice(0,k+1).includes(x),isCurrent=x===seq[k];s+=`<circle cx="${cx}" cy="${cy}" r="20" fill="${isCurrent?colors[0]:visited?soft[0]:'var(--diagram-surface,#fffefa)'}" stroke="${visited?colors[0]:'var(--diagram-line,#c3d0c2)'}" stroke-width="${isCurrent?2:1}"/><text x="${cx}" y="${cy+5}" font-size="15" text-anchor="middle" fill="${isCurrent?'var(--diagram-solid-text,#fff)':colors[0]}">${fmt(x)}</text>`;});
        s+=`<text x="380" y="132" text-anchor="middle" class="math-label" font-size="27" fill="var(--diagram-ink,#234e40)">${key==='add6'?`${k}[${a}]`:`a${['⁰','¹','²','³','⁴','⁵','⁶'][k]||'^'+k}`}</text><text x="380" y="162" text-anchor="middle" font-size="13" fill="var(--diagram-muted,#61736c)">${k===ord?'第一次返回单位元':k===0?'从单位元出发':'重复同一个元素'}</text>`;
