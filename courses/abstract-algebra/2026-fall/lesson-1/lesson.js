...
(()=>{
  ...
  function renderCriteria(){
    ...
    function draw(){const c=criteria[chosen];$('criteria-tabs').innerHTML=criteria.map((x,i)=>`<button type="button" data-criterion="${i}" aria-pressed="${chosen===i}">${x.name}</button>`).join('');$('criteria-condition').textContent=step<0?'':c.condition;$('criteria-condition').hidden=step<0;$('criteria-proof').innerHTML=`<p class="label"><span>${c.ref}</span> · <span>${step<0?'定理阐述':'证明'}</span></p>${step<0?`<div class="theorem-statement">${statements[chosen]}</div>`:`<p class="criteria-step">${c.steps[step]}</p>`}`;$('criteria-back').disabled=step<0;$('criteria-back').textContent=step===0?'← 返回定理':'← 上一步';$('criteria-next').textContent=step<0?'开始证明 →':'下一步 →';$('criteria-next').disabled=step===c.steps.length-1;$('criteria-count').textContent=step<0?'先读清条件与结论':`${step+1} / ${c.steps.length}`;$('criteria-note').textContent=c.note;$('criteria-note').hidden=step<0;}
