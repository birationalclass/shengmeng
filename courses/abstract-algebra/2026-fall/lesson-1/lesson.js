...
(()=>{
  ...
  allSections.push({id:'associativity-game',nav:'拉丁方数独',title:'用结合律补全乘法表',lead:'从 2×2 到 9×9，借助技能卡逐步推导。',formula:'(ab)c = a(bc)',definition:'每行每列无重复，并满足结合律。',prompt:'线索唯一确定每关的循环群乘法表。',source:'扩展 C · 运算、映射与乘法表',label:'结合律实验室 · 八个关卡',render:()=>window.AssociativitySudoku.render(scene)});
  textbookSections['1.2'].items.splice(textbookSections['1.2'].items.indexOf('check'),0,'associativity-game');
  ...
  function renderCriteria(){
    ...
    criteria.push({"name": "扩展：运算与映射", "ref": "扩展 C · 运算、映射与乘法表", "condition": "固定 a：右乘看列，左乘看行；有限 G 上，满射 ⇔ 单射 ⇔ 双射。", "steps": ["设 G 为非空集合，二元运算就是映射 m : G × G → G，记 m(x,y) = xy。固定 a ∈ G，定义右乘映射 mₐ : G → G，mₐ(x) = m(x,a) = xa；左乘映射 Lₐ : G → G，Lₐ(x) = m(a,x) = ax。mₐ 与 Lₐ 中的 a 均为下标，用于标记固定的乘数。", "固定 a。对每个 b ∈ G，xa = b 有解 ⇔ 对每个 b ∈ G，存在 x ∈ G 使 mₐ(x) = b ⇔ mₐ(G) = G ⇔ mₐ 满射。乘法表按行元素乘列元素读取；固定第 a 列、遍历各行，得到的正是全部 mₐ(x)。故满射等价于该列取遍 G。", "mₐ 单射 ⇔ 对任意 x,y ∈ G，xa = ya ⇒ x = y ⇔ 第 a 列不同位置的元素两两不同。这是对固定右因子 a 的消去律；对所有 a 都成立时，才是整个运算的右消去律。这里不需要 G 有限。", "左右对偶：对每个 b ∈ G，ax = b 有解 ⇔ Lₐ 满射 ⇔ 第 a 行取遍 G；Lₐ 单射 ⇔ ax = ay ⇒ x = y ⇔ 第 a 行元素两两不同。下表 m(u,v) = u 的第 1 列取遍 G，但第 1 行只有 1：左右条件不能混淆。", "有限情形的证明（单射 ⇒ 满射）：设 |G| = n。任意映射 f : G → G 若单射，则 n 个输入给出 n 个两两不同的输出；这些输出都在仅有 n 个元素的 G 中，故 f(G) = G。", "有限情形的证明（满射 ⇒ 单射）：若 f : G → G 不单射，则至少两个不同输入给出同一输出，故 n 个输入至多给出 n − 1 个不同输出，不可能取遍 G。因此在有限 G 上，满射 ⇔ 单射 ⇔ 双射。应用于 mₐ 或 Lₐ，便得到一列或一行“取遍 G ⇔ 元素两两不同”。", "不论 G 是否有限，对每个 b，xa = b 恰有一个解 ⇔ mₐ 双射：有解就是满射，至多一个解就是单射。ax = b 与 Lₐ 同理。若 G 有限，单独要求“对每个 b 有解”或“该列（行）无重复”便已保证每个 b 恰有一个解。", "有限性不可省略：在 G = ℕ₀ = {0,1,2,…} 上取 m(x,y) = x + y，固定 a = 1。m₁(x) = x + 1 单射，但 0 不在像中，故不满射。其第 1 列 1,2,3,… 两两不同，却没有取遍 G；此例甚至满足结合律。", "与扩展 A、B 的联系：扩展 A 的条件是结合律、mₐ₀ 满射，以及存在 e 使 Lₐ₀(e) = a₀；结论是 mₑ = id_G。扩展 B 则交换左右：Lₐ₀ 满射且 mₐ₀(e) = a₀，推出 Lₑ = id_G。id_G 表示恒等映射。", "因此，非空有限 G 上的运算若满足结合律，且乘法表每行、每列元素都两两不同，则所有 mₐ、Lₐ 都满射，两类方程对所有 a,b 都有解；由定理 1.2.4，G 是群。只有某一列无重复并不够。映射与行列的对应关系本身不需要结合律。"], "note": "这里 G 只是非空集合，不预先假设它是群。除最后的群判别及引用扩展 A、B 外，均不需要结合律。", "mappingExample": true});
    statements.push("<p>把运算写成 m : G × G → G。固定 a 后，mₐ(x) = m(x,a) = xa 是右乘映射，Lₐ(x) = m(a,x) = ax 是左乘映射。方程对所有右端有解对应满射，消去律对应单射；在有限集合上二者等价。</p><p>这里 G 只是非空集合，不预先假设它是群。除最后的群判别及引用扩展 A、B 外，均不需要结合律。</p>");
    criteria[5].steps.splice(9,0,"结合左右两侧：有限 G 上，所有 mₐ、Lₐ 都满射 ⇔ 所有 mₐ、Lₐ 都单射 ⇔ 乘法表每行、每列都恰好出现一次 G 的每个元素。这称为拉丁方条件，可比喻为“数独式的行列条件”，但不要求数独的小宫格条件；阶数也不必是 9。","拉丁方不自动给出群：在 G = {0,1,2} 上定义 m(x,y) = x − y（模 3）。表中每行、每列都是 0,1,2 的排列，但 m(m(0,0),1) = 2，而 m(0,m(0,1)) = 1，故不满足结合律。满足拉丁方条件的运算称为拟群；有结合律的拟群才是群。");
    criteria[3].counterexample={rule:'uv = u',rows:[[0,0],[1,1]]};
    criteria[4].counterexample={rule:'uv = v',rows:[[0,1],[0,1]]};
    function mappingTable(c){
      if(!c.mappingExample)return '';
      if(step===10){
        const values=[0,1,2];
        return `<figure class="identity-counterexample mapping-example"><table><caption><span>拉丁方反例：m(x,y) = x − y（模 3）</span></caption><thead><tr><th scope="col">m</th>${values.map(v=>`<th scope="col">${v}</th>`).join('')}</tr></thead><tbody>${values.map(u=>`<tr><th scope="row">${u}</th>${values.map(v=>`<td>${(u-v+3)%3}</td>`).join('')}</tr>`).join('')}</tbody></table><figcaption>每行、每列都无重复，但结合律不成立。</figcaption></figure>`;
      }
      if(![1,2,3].includes(step))return '';
      const left=step===3,header='<tr><th scope="col">·</th><th scope="col">0</th><th scope="col" class="'+(!left?'mapping-selected':'')+'">1</th></tr>';
      const body=[0,1].map(u=>`<tr><th scope="row" class="${left&&u===1?'mapping-selected':''}">${u}</th>${[0,1].map(v=>`<td class="${(left?u===1:v===1)?'mapping-selected':''}">${u}</td>`).join('')}</tr>`).join('');
      return `<figure class="identity-counterexample mapping-example"><table><caption><span>例：G = {0,1}，m(u,v) = u，a = 1</span></caption><thead>${header}</thead><tbody>${body}</tbody></table><figcaption>${left?'左乘 L₁：读第 1 行，得到 1,1；既非满射，也非单射。':'右乘 m₁：读第 1 列，得到 0,1；既是满射，也是单射。'}</figcaption></figure>`;
    }
    ...
    function draw(){const c=criteria[chosen];$('criteria-tabs').innerHTML=criteria.map((x,i)=>`<button type="button" data-criterion="${i}" aria-pressed="${chosen===i}">${x.name}</button>`).join('');$('criteria-condition').textContent=step<0?'':c.condition;$('criteria-condition').hidden=step<0;$('criteria-proof').innerHTML=`<p class="label"><span>${c.ref}</span> · <span>${step<0?'定理阐述':'证明'}</span></p>${step<0?`<div class="theorem-statement">${statements[chosen]}</div>`:`${counterexampleTable(c)}${mappingTable(c)}<p class="criteria-step">${c.steps[step]}</p>`}`;$('criteria-back').disabled=step<0;$('criteria-back').textContent=step===0?'← 返回定理':'← 上一步';$('criteria-next').textContent=step<0?'开始证明 →':'下一步 →';$('criteria-next').disabled=step===c.steps.length-1;$('criteria-count').textContent=step<0?'先读清条件与结论':`${step+1} / ${c.steps.length}`;$('criteria-note').textContent=c.note;$('criteria-note').hidden=step<0;}
