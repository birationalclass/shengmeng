...
(()=>{
  ...
  window.LessonNotebookContent={
    ...
    '1.2':[
      ...
      item("criteria",["扩展 A · 定理 1.2.4", "Extension A · Theorem 1.2.4"],["由一个固定元素构造右单位元", "A right identity from one fixed element"],"a₀e = a₀,  ∀b ∃y: ya₀ = b<br>⇒ ∀b: be = b",["设 S 为非空集合，具有满足结合律的二元运算 S × S → S。若存在固定 a₀ ∈ S，使 a₀x = a₀ 在 S 中有解，且对每个 b ∈ S，ya₀ = b 在 S 中有解，则 a₀x = a₀ 的每个解都是 S 的右单位元。", "Let S be a nonempty set with an associative binary operation S × S → S. If there is a fixed a₀ ∈ S such that a₀x = a₀ is solvable in S and ya₀ = b is solvable in S for every b ∈ S, then every solution of a₀x = a₀ is a right identity of S."],{"criterion": 3, "extension": true, "detail": ["两项条件中的 a₀ 是同一个固定元素；先选 a₀ 与 e，再任取 b。所有解都要求属于 S。结论只保证单侧单位元存在，不保证另一侧、唯一性或群结构。", "The same fixed a₀ occurs in both assumptions. Choose a₀ and e before taking an arbitrary b. All solutions must lie in S. The conclusion gives a one-sided identity, not an identity on the other side, uniqueness or a group structure."]}),
      item("criteria",["扩展 B · 对偶命题", "Extension B · Dual statement"],["由一个固定元素构造左单位元", "A left identity from one fixed element"],"ea₀ = a₀,  ∀b ∃x: a₀x = b<br>⇒ ∀b: eb = b",["设 S 为非空集合，具有满足结合律的二元运算 S × S → S。若存在固定 a₀ ∈ S，使 ya₀ = a₀ 在 S 中有解，且对每个 b ∈ S，a₀x = b 在 S 中有解，则 ya₀ = a₀ 的每个解都是 S 的左单位元。", "Let S be a nonempty set with an associative binary operation S × S → S. If there is a fixed a₀ ∈ S such that ya₀ = a₀ is solvable in S and a₀x = b is solvable in S for every b ∈ S, then every solution of ya₀ = a₀ is a left identity of S."],{"criterion": 4, "extension": true, "detail": ["两项条件中的 a₀ 是同一个固定元素；先选 a₀ 与 e，再任取 b。所有解都要求属于 S。结论只保证单侧单位元存在，不保证另一侧、唯一性或群结构。", "The same fixed a₀ occurs in both assumptions. Choose a₀ and e before taking an arbitrary b. All solutions must lie in S. The conclusion gives a one-sided identity, not an identity on the other side, uniqueness or a group structure."]}),
      item("criteria",["扩展 C · 运算、映射与乘法表", "Extension C · Operations, maps and tables"],["用映射理解运算与消去律", "Operations and cancellation through maps"],"m : G × G → G<br>m<sub>a</sub>(x) = m(x,a) = xa<br>L<sub>a</sub>(x) = m(a,x) = ax",["把运算写成 m : G × G → G。固定 a 后，mₐ(x) = m(x,a) = xa 是右乘映射，Lₐ(x) = m(a,x) = ax 是左乘映射。方程对所有右端有解对应满射，消去律对应单射；在有限集合上二者等价。", "Write the operation as m : G × G → G. Fixing a gives right multiplication mₐ(x) = m(x,a) = xa and left multiplication Lₐ(x) = m(a,x) = ax. Solvability for every target corresponds to surjectivity; cancellation corresponds to injectivity. These are equivalent on finite sets."],{"criterion": 5, "extension": true, "groupSudoku": true, "detail": ["这里 G 只是非空集合，不预先假设它是群。除最后的群判别及引用扩展 A、B 外，均不需要结合律。 有限情形中，每行、每列恰好出现一次所有元素，称为拉丁方条件；它类似数独的行列规则，但没有小宫格要求。拉丁方条件加结合律，才给出群。", "Here G is only a nonempty set, not assumed to be a group. Associativity is needed only for the final group criterion and the references to Extensions A and B. In the finite case, each row and column containing every element exactly once is the Latin square condition: like Sudoku rows and columns, without subgrid requirements. The Latin square condition together with associativity gives a group."]}),
      item("associativity-game",["扩展 C · 结合律小游戏", "Extension C · Associativity game"],["拉丁方数独：2×2 到 9×9", "Latin-square puzzles: 2×2 to 9×9"],"m(m(a,b),c) = m(a,m(b,c))",["共八关，每关用 1 到 n 标记 n 个元素。给定部分乘法表，要求每行每列恰好出现一次每个元素，并满足结合律。用单位元、逆元、消去律与结合律技能卡辅助推导。", "Eight levels label n elements by 1 through n. Complete each partially given multiplication table so that every row and column contains each label exactly once and associativity holds. Use identity, inverse, cancellation and associativity skill cards."],{"extension": true, "groupSudoku": true, "detail": ["2×2、3×3 从行列规则开始；4×4 到 9×9 结合行列条件与结合律推导。空缺分散在各处，每关均附逐格唯一性证明。这里没有普通数独的宫格限制；6×6、8×8 分别采用非交换群 S₃、D₄；4×4 用 C₂×C₂，9×9 用 C₃×C₃，其余为循环群。单位元不标为 1。", "Begin with row/column rules at 2×2 and 3×3; combine these with associativity from 4×4 to 9×9. Blanks are scattered and every level includes a cell-by-cell uniqueness proof. No ordinary Sudoku block constraints apply; orders 6 and 8 use nonabelian S₃ and D₄, order 4 uses C₂×C₂, order 9 uses C₃×C₃, and the others are cyclic. The identity is not labeled 1."]}),
