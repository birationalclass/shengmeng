...
(()=>{
  ...
  const scheduleTranslations={};document.querySelectorAll('#schedule tbody tr').forEach((row,i)=>{const pair=window.CourseScheduleEnglish[i];if(!pair)return;const title=row.querySelector('h3'),focus=row.querySelector('.focus');scheduleTranslations[title.textContent.trim()]=pair[0]+(title.querySelector('a')?' ↗':'');scheduleTranslations[focus.textContent.trim()]=pair[1];});window.CourseLanguage.add(scheduleTranslations);
  ...
  function labels(){
    ...
    $('portal-fullscreen').hidden=view!=='lesson'||phone.matches;$('portal-fullscreen').textContent=document.fullscreenElement?t('退出全屏','Exit fullscreen'):t('全屏','Fullscreen');
  ...
  $('portal-fullscreen').onclick=async()=>{if(phone.matches||view!=='lesson')return;try{if(document.fullscreenElement)await document.exitFullscreen();else await document.documentElement.requestFullscreen?.();}catch{}labels();};
