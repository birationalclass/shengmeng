...
window.LessonNotebookContent={[id]:entries.map(e=>({topic:e.id,ref:[`§${id}`,`§${id}`],title:e.title,formula:formula(e.tex),text:e.text,detail:book.optional?['拓展阅读 · 不增加既定课表中的必讲课时。','Optional reading · outside the required scheduled teaching.']:undefined}))};
...
$('section-title').textContent=t(e.title);$('experiment-label').textContent=`§${id} · ${t(e.title)}`;$('intro-title').textContent=t(book.title);document.title=`§${id} ${t(book.title)} · ${t(['抽象代数 I','Abstract Algebra I'])}`;
...
scene.innerHTML=`<div class="notebook-visual">${e.visual?'<div class="group-demo"></div>':`<div class="group-focus">${formula(e.tex)}<p>${t(e.title)}</p></div>`}</div><section class="notebook-exposition">${e.proof?'<div class="group-proof"></div>':''}${e.example?`<article class="group-example"><h3>${t(['例子与应用','Example and application'])}</h3><p>${t(e.example)}</p></article>`:''}${e.extraProof?`<details class="group-detail"><summary>${t(['补充论证','Supporting argument'])}</summary>${e.extraProof.map(s=>`<p>${t(s)}</p>`).join('')}</details>`:''}${e.warning?`<aside class="group-warning"><h3>${t(['注意条件','Check the hypotheses'])}</h3><p>${t(e.warning)}</p></aside>`:''}<p class="group-recap">${t(['课堂任务：用自己的话解释左侧结论；指出一个关键条件，并举例说明。','Class task: explain the statement in your own words, identify a key hypothesis, and give an example.'])}</p></section>`;
