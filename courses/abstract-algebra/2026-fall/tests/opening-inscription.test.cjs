...
for(const [w,h] of [[390,844],[430,932],[768,1024],[1000,1000]])assert.equal(I.scale(w,h),1);
...
p.mark(portrait,new Float32Array([0,1,1]));p.mark(closing);
p.update(portrait,figure);assert.deepEqual([...p.attributes],[0,0,1,0,1,0]);
...
p.update(capture,closing);assert.deepEqual([...p.attributes],[0,1,.75,1,.75,1],'interrupted morph retains each grain’s current lettering weight');
const second=new Float32Array(9);p.capture(capture,closing,.4,second);
p.update(second,figure);for(const [i,value] of [0.4,0.85,0.85].entries())assert.ok(Math.abs(p.attributes[2*i]-value)<1e-6);
p.update(figure,figure);assert.ok(p.attributes.every(v=>v===0),'ordinary sand scenes are unaffected');
console.log('PASS: phone rendering preserved, landscape coverage matched, resize and interrupted morphs continuous, non-text grains unaffected');
