...
for(const ids of [[3],[3,4],[4,3],[8,3,5]])for(const phaseOffset of [0,.37]){
  ...
  for(let t=0;t<timeline.duration;t+=100){
    ...
    assert.ok(view.zoom>=1&&view.zoom<=2.350001);
    if(ids[s.from]===3&&ids[s.to]===3||!s.moving&&ids[s.scene]===3){
      assert.ok(view.zoom>=1&&view.zoom<=1.4+1e-12,'Lie-group zoom is limited to 40%');assert.deepEqual(view.target,[0,0,0]);
  ...
  assert.ok(Math.abs(maxZoom-1.4)<1e-12,'each Lie-group viewing interval reaches the requested 40%');
...
console.log('PASS: Lie-group zoom stays within 1x–1.4x, bounded Z-axis motion, reverse/custom order/single-scene loops and smooth joins');
