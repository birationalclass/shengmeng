...
for(const ids of [[3],[3,4],[4,3],[8,3,5]])for(const phaseOffset of [0,.37]){
  ...
  assert.ok(Math.abs(maxZoom-3)<1e-12,'each Lie-group viewing interval reaches the requested 300%');
...
console.log('PASS: Lie-group zoom stays within 1x–3x, bounded Z-axis motion, reverse/custom order/single-scene loops and smooth joins');
