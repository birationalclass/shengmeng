...
for(const ids of [[3],[3,4],[4,3],[8,3,5]])for(const phaseOffset of [0,.37]){
  ...
  for(let t=0;t<timeline.duration;t+=100){
    ...
    if(ids[s.from]===3&&ids[s.to]===3||!s.moving&&ids[s.scene]===3){
      assert.equal(view.zoom,1,'Lie-group hold never zooms');assert.deepEqual(view.target,[0,0,0]);
...
console.log('PASS: Lie-group view stays whole at 1x, bounded Z-axis motion, reverse/custom order/single-scene loops and smooth joins');
