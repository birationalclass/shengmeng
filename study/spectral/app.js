...
document.addEventListener('keydown',e=>{if(!['Enter','ArrowRight','ArrowLeft'].includes(e.key)||$('#references').open||$('#formulaDialog').open||e.target.closest('input,select,textarea,[contenteditable=true]')||e.key==='Enter'&&e.target.closest('button,a'))return;e.preventDefault();e.stopImmediatePropagation();if(e.key==='ArrowLeft')retreatSlide();else advanceSlide();},true);
...
function advanceSlide(){if(isDoubleComplexView()&&state.initialReveal<6){advanceInitial(1);return;}if(mainSlide()<3)goSlide(mainSlide()+1);}
...
function initialConcept(){return [null,'delta1','delta2','square1','anticommute','total','totalmap'][state.initialReveal];}
function advanceInitial(direction){state.initialReveal=Math.max(0,Math.min(6,state.initialReveal+direction));state.seenH=state.initialReveal>=1;state.seenV=state.initialReveal>=2;state.effect=initialConcept();state.pinned=null;state.pinnedKey=null;render();const pane=$('.explanation'),card=$(`[data-build="${state.initialReveal}"]`);if(!card){pane.scrollTop=0;return;}const r=card.getBoundingClientRect(),b=pane.getBoundingClientRect();if(r.bottom>b.bottom)pane.scrollTop+=r.bottom-b.bottom+12;else if(r.top<b.top)pane.scrollTop-=b.top-r.top+12;}
...
function applyConcept(concept,hover=false,source=null){
 ...
 const targets={square1:'.relation-route',square2:'.relation-route',anticommute:'.relation-route',delta1:'.arrow.h,.continuation[data-concept=delta1]',delta2:'.arrow.v,.continuation[data-concept=delta2]',differential:'.arrow,.continuation[data-concept=delta1],.continuation[data-concept=delta2],.continuation[data-concept=differential]',space:'.node:not(.outside-quadrant) .node-bg',page:'.node:not(.outside-quadrant) .node-bg',quotient:'.node:not(.muted):not(.outside-quadrant) .node-bg',totalmap:'.diag-box,.diag,#diagram-edges>g:not(.context-edge) .arrow,#diagram-edges>g:not(.context-edge) .continuation',total:'.diag-box,.diag,.node:not(.muted):not(.outside-quadrant) .node-bg,.continuation[data-concept=total]',filtration:'.diag-box,.node:not(.muted):not(.outside-quadrant) .node-bg',cycles:'[data-concept=cycles]',boundaries:'[data-concept=boundaries]',cohomology:'.node:not(.muted):not(.outside-quadrant) .node-bg',comparison:'.node:not(.zero) .node-bg,.diag-box'};
 if(concept){
...
function setupDoubleComplex(){
 ...
 $('#next').textContent=state.initialReveal<6?ui('下一步 →','Next step →'):ui('下一页 →','Next slide →');$('#prev').textContent=state.initialReveal?ui('← 上一步','← Previous step'):ui('← 上一页','← Previous slide');
 $('#fragmentTrack').innerHTML=math([raw`K^{p,q}`,raw`\delta_1`,raw`\delta_2`,raw`\delta_1^2=\delta_2^2=0`,raw`\delta_1\delta_2+\delta_2\delta_1=0`,raw`C^\bullet`,raw`D`][state.initialReveal]);
...
function fixedDiagram(){
 ...
 if(m==='initial'){
  total=['total','totalmap'].includes(state.effect);h=total?state.effect==='totalmap':state.seenH;v=total?state.effect==='totalmap':state.seenV;
