...
import {hodgeSetup} from './hodge.js?v=156';
import {leraySetup} from './leray.js?v=156';
import {createDifferentialSweep} from './differential-sweep.js?v=120';
import {filteredSubcomplexExposition} from './filtered-subcomplex.js?v=156';
...
import {numberedPages} from './reading-pages.js?v=156';
...
import {translatePage,language,toggleLanguage} from './language.js?v=156';
import {operationMarkup,viewNames,actionNames,totalDegreeTex} from './workbench.js?v=90';
import {createFilteredView} from './filtered-view.js?v=156';
import {createPageEvolution} from './page-evolution.js?v=156';
import {createNotebookMotion} from './notebook-motion.js?v=156';
...
const markSettings=createStatementMarkSettings({language});
...
const readingSections=[['initial:0','learn:6'],['learn:5','converge:0','learn:3','learn:4','converge:3'],['converge:5'],['converge:4']];
...
const formulas=(fs,concepts=[],number='',module='',step=0)=>numberedPages(module,step,fs.length).map((page,index)=>{
 ...
 const content=page.indices.map(i=>{
  ...
  if(module==='converge'&&step===5)return hodgeSetup({math,t:ui,page:i-1})+formula;
...
function sectionTwoMarkup(section=2){
 const heading=statementHeading(section===2?statementMeta('learn',5):statementMeta('converge',section===3?5:4),String(section)).replace('data-select-statement','data-select-section').replace('data-toggle-statement','data-toggle-section');
...
function render(updateControls=true){
 ...
 markSettings.sync();renderPersistentDiagram();if(updateControls)controls();inspect();renderWorkspaceState();fitDiagram();translatePage();readingRail.sync();notebookMotion.sync();window.spectralState={...state,language:language()};syncInitialEntrance();syncPlayback();window.spectralFullscreen?.sync();
}
...
function statementMeta(module=state.module,step=state.step){
 ...
 if(module==='initial'&&step===0)return {...meta,kind:'§',number:'1',name:ui('第一象限双复形','First-quadrant double complex'),symbol:null};
 ...
 if(module==='converge'&&step===5)return {kind:'§',number:'3',name:ui('Hodge 分解','Hodge decomposition'),concepts:[]};
 if(module==='converge'&&step===4)return {kind:'§',number:'4',name:ui('代数几何应用 · Leray 谱序列','Algebraic geometry · Leray spectral sequence'),concepts:[]};
...
function consequence(formula,concept,attribute=''){
 return `<div class="reading-consequence" ${attribute}><span class="consequence-cue">${statementMark('P',language()==='en')}</span>${block(formula,concept)}</div>`;
}
