...
export const lessons=[
 ...
 {title:'E₁',tag:'THE FIRST PAGE',f:[t`E_1^{p,q}:=\frac{Z_1^{p,q}}{F^{p+1}C^{p+q}+B_0^{p,q}}`,t`d_1^{p,q}:E_1^{p,q}\longrightarrow E_1^{p+1,q}`,t`d_1^{p,q}[a]_1:=[\delta_1a]_1,\qquad a\in K^{p,q},\ \delta_2a=0`],text:'E₁ 由滤过商定义，自然同构于逐列 d₀ 上同调；δ₁ 诱导 d₁。',note:'d₁ 的定义域与目标都在 E₁ 层。',proof:'反交换关系保证 δ₁ 将纵向闭元映为闭元，并将纵向边界映为边界。因此 d₁ 良定义。'},
 {title:'一般页：滤过商',tag:'THE FILTERED QUOTIENT',f:[t`Z_r^{p,q}:=F^pC^{p+q}\cap D^{-1}(F^{p+r}C^{p+q+1})`,t`B_r^{p,q}:=F^pC^{p+q}\cap D(F^{p-r}C^{p+q-1})`,t`\begin{gathered}E_r^{p,q}:=\frac{Z_r^{p,q}}{Z_{r-1}^{p+1,q-1}+B_{r-1}^{p,q}}\\(r\ge1)\end{gathered}`,t`\begin{gathered}d_r^{p,q}:E_r^{p,q}\longrightarrow E_r^{p+r,q-r+1}\\ d_r^{p,q}[a]_r:=[Da]_r,\quad a\in Z_r^{p,q}\end{gathered}`],text:'Eᵣ 定义为原滤过复形中的商空间；D 诱导 dᵣ。',note:'Eᵣ 与前一页上同调自然同构是构造的性质；新的微分 dᵣ 仍需原滤过复形。',proof:'D 将分子与分母分别映入目标分子与分母，从而诱导 dᵣ。构造定理证明 Eᵣ 自然同构于前一页的上同调。'}
];
