...
export const convergence=[
 {title:'自然过渡映射与逐位置稳定',f:[t`\pi_r^{p,q}:\ker d_r^{p,q}\twoheadrightarrow E_{r+1}^{p,q}`,t`r>\max\{p,q+1\}`,t`\begin{gathered}d_r^{p,q}=0,\\ d_r^{p-r,q+r-1}=0,\end{gathered}`,t`E_r^{p,q}\xrightarrow{\sim}E_{r+1}^{p,q}\xrightarrow{\sim}\cdots`],text:'第一象限保证此位置的入射与出射微分同时为零，后续各页自然同构。',note:'',proof:'出射靶的第二指标 q-r+1 为负；入射源的第一指标 p-r 为负。对每个 s≥r，两个微分都为零，故 E_{s+1}^{p,q}≅E_s^{p,q}/0，自然同构于 E_s^{p,q}。'},
