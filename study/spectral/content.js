...
export const lessons=[
 ...
 {title:'一般页：D 在不同商上诱导 dᵣ',tag:'FILTERED CYCLES AND BOUNDARIES',f:[t`Z_r^{p,q}:=F^pC^{p+q}\cap D^{-1}(F^{p+r}C^{p+q+1})`,t`B_r^{p,q}:=F^pC^{p+q}\cap D(F^{p-r}C^{p+q-1})`,t`E_r^{p,q}:=\frac{Z_r^{p,q}}{Z_{r-1}^{p+1,q-1}+B_{r-1}^{p,q}}\quad(r\ge1)`,t`\begin{gathered}d_r^{p,q}:E_r^{p,q}\longrightarrow E_r^{p+r,q-r+1}\\ d_r^{p,q}[a]_r:=[Da]_r,\quad a\in Z_r^{p,q}\end{gathered}`],text:'第 r 页的微分仍来自同一个 D。分子要求微分至少落到右边第 r 列；分母控制哪些代表元在本页等价。',note:'调节 r，金色箭头的位移始终是 (r,1−r)。一般页先由滤过复形定义；不是从 E₁ 的维数猜出来。',proof:'D 将分子映到目标分子，并将分母映到目标分母，因而诱导 dᵣ。继承 D²=0 后，再由核与像的商得到 Eᵣ₊₁≅H(Eᵣ,dᵣ)。完整商空间证明见讲义 §3。'}
];
