...
const t=String.raw;
// McCleary, Theorem 2.6 proof, printed p. 34: subspaces of total cochains.
export const filteredSubspaces=[
 t`\begin{gathered}Z_r^{p,q}:=F^pC^{p+q}\cap D^{-1}(F^{p+r}C^{p+q+1})\\(r\ge0)\end{gathered}`,
 t`\begin{gathered}B_r^{p,q}:=F^pC^{p+q}\cap D(F^{p-r}C^{p+q-1})\\(r\ge0)\end{gathered}`
];
export const lessons=[
 ...
 {title:'一般页：滤过商',tag:'THE FILTERED QUOTIENT',f:[...filteredSubspaces,t`\begin{gathered}E_r^{p,q}:=\frac{Z_r^{p,q}}{Z_{r-1}^{p+1,q-1}+B_{r-1}^{p,q}}\\(r\ge1)\end{gathered}`,t`\begin{array}{rcl}d_r^{p,q}:E_r^{p,q}&\longrightarrow&E_r^{p+r,q-r+1}\\[.4em][${representativeIn("a",t`Z_r^{p,q}`)}]_r&\longmapsto&[Da]_r\end{array}`],text:'Eᵣ 定义为原滤过复形中的商空间；D 诱导 dᵣ。',note:'Eᵣ 与前一页上同调自然同构是构造的性质；新的微分 dᵣ 仍需原滤过复形。',proof:'D 将分子与分母分别映入目标分子与分母，从而诱导 dᵣ。构造定理证明 Eᵣ 自然同构于前一页的上同调。'},
