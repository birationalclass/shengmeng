...
export function createTotalTrace({host,math,point,degree}){
 ...
 function play(effect,force=false){
  ...
  function draw(t){
   const first=ease((t-180)/760);flows.forEach((el,i)=>glow(el,t,isMixed?(i<2?360:1420):i<2?(isSquare?360:180):1910,isMixed?800:isSquare?(i<2?800:850):760));
   if(isMixed){
    const split=ease((t-180)/180),firstMove=ease((t-360)/800),secondMove=ease((t-1420)/800);
    const combine=ease((t-2520)/520),morph=ease((t-3040)/360);
    ...
    dots.forEach((_,i)=>{
     const reached=mix(mix(starts[i],intermediates[i],firstMove),m,secondMove);
     reached[0]+=(i===0?1:-1)*12*ease((secondMove-.76)/.24);
     dot(i,mix(reached,m,combine),split*(1-morph),7*(1-combine)+.8*combine);
    });
    const zero=zeros[0],scale=.08+.92*morph;
    zero.setAttribute('opacity',morph);
    zero.setAttribute('transform',`translate(${m[0]} ${m[1]}) scale(${scale}) translate(${-m[0]} ${-m[1]})`);
    result.style.opacity=String(ease((t-3280)/360));
   }else if(!isSquare){
