...
export function createTotalTrace({host,math,point,degree}){
 ...
 function play(effect,force=false){
  ...
  layer.innerHTML=`<g class="total-flow" fill="none" stroke-width="2.7" stroke-linecap="round">${[ [a,h],[a,v],...(isSquare?[[h,hh],[h,m],[v,m],[v,vv]]:[]) ].map(([s,t],i)=>{const d=Math.hypot(t[0]-s[0],t[1]-s[1]),pad=s[0]===t[0]?24:39;return `<path d="M${s[0]+(t[0]-s[0])*pad/d},${s[1]+(t[1]-s[1])*pad/d} L${t[0]-(t[0]-s[0])*pad/d},${t[1]-(t[1]-s[1])*pad/d}" stroke="${i%2?colors[1]:colors[0]}" opacity="0"/>`;}).join('')}</g>${isSquare?`<circle class="total-seed" cx="${a[0]}" cy="${a[1]}" r="7" fill="var(--gold)"/>`:""}${Array.from({length:isSquare?4:2},(_,i)=>`<circle class="total-dot" r="7" fill="${colors[i]}" stroke="#effaf799" stroke-width=".7"/>`).join('')}${(isSquare?[hh,m,vv]:[sum]).map(([x,y])=>`<text class="total-zero" x="${x}" y="${y+1}" text-anchor="middle" dominant-baseline="central" fill="var(--gold)" stroke="#10252e" stroke-width="4" paint-order="stroke" font-family="KaTeX_Main,Georgia,serif" font-size="28" opacity="0">0</text>`).join('')}`;
  host.append(layer);
  ...
  function draw(t){
   const first=ease((t-180)/760);flows.forEach((el,i)=>glow(el,t,i<2?(isSquare?360:180):1910,isSquare?(i<2?800:850):760));
   if(!isSquare){
   ...
   layer.dataset.phase=isSquare?(t<360?'first-split':t<1160?'first-differential':t<1650?'first-image':t<1910?'second-split':t<2760?'second-differential':t<2960?'second-image':t<3860?'cancellation':'complete'):(t<940?'first-differential':t<1250?'summands':t<2120?'second-stage':t<2910?'combination':'complete');
  }
  const duration=isSquare?4100:3100;
  if(reduced()){draw(duration);running=false;return;}
  const start=performance.now();draw(0);const tick=now=>{if(run!==token)return;draw(now-start);if(now-start<duration)frame=requestAnimationFrame(tick);else running=false;};frame=requestAnimationFrame(tick);
 }
