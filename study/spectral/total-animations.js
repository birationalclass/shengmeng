...
export function createTotalTrace({host,math,point,degree}){
 ...
 function play(effect,force=false){
  ...
  // Pause on the incoming paths, before their common endpoint. The midpoint
  // stays inside the same target K and is the centre of addition and zero.
  const mixedStops=[[m[0],m[1]+14],[m[0]-14,m[1]]],additionCenter=mix(mixedStops[0],mixedStops[1],.5);
