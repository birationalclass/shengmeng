...
// Total differential: its two components stop in their target K-summands.
// D²: the mixed terms cancel in the same K; the pure terms vanish separately.
export function createTotalTrace({host,point,origin=()=>({p:1,q:1})}){
 const ns='http://www.w3.org/2000/svg';let layer=null,frame=0,kind=null,running=false,token=0;
 const reduced=()=>matchMedia('(prefers-reduced-motion:reduce)').matches;
 ...
 function clear(){
  ...
  for(const old of[layer].filter(Boolean)){old.removeAttribute('id');if(reduced())old.remove();else{const fade=old.animate([{opacity:getComputedStyle(old).opacity},{opacity:0}],{duration:visualMotion().exit,easing:visualMotion().easing,fill:'forwards'});fade.finished.then(()=>old.remove(),()=>{});}}
  layer=null;
 }
 function play(effect,force=false){
  ...
  function draw(t){
   ...
   if(isMixed){
    ...
    // D(a) has two components in distinct target summands. Each stays put.
    dot(0,mix(a,h,first),ease(t/180));
    dot(1,mix(a,v,first),ease(t/180));
   }else{
    const split=ease((t-180)/180),firstMove=ease((t-360)/800);
   ...
   layer.dataset.phase=isMixed?(t<360?'split':t<1160?'first-maps':t<1420?'intermediate':t<2180?'mixed-composites':t<2800?'braking':t<3100?'opposite-elements':t<3580?'addition':t<4220?'zero-morph':'complete'):isSquare?(t<360?'first-split':t<1160?'first-differential':t<1650?'first-image':t<1910?'second-split':t<2760?'second-differential':t<2960?'second-image':t<3860?'cancellation':'complete'):(t<940?'first-differential':'complete');
  }
  const duration=isMixed?4450:isSquare?4100:1150;
