...
const choices={
 sand:['金沙','Golden sand','深褐底与暖金细砂 · 默认','Dark sandstone and warm gold · Default'],
 editorial:['A · 墨色书页','A · Editorial notebook','留白与清晰层次','Space and hierarchy'],
...
export function createPanelStyle({language}){
 ...
 let current='sand';try{const value=localStorage.getItem(key);if(choices[value])current=value;}catch{}
 ...
 function apply(){root.dataset.panelStyle=current;document.querySelector('meta[name="theme-color"]')?.setAttribute('content',getComputedStyle(root).getPropertyValue('--bg').trim());}
 function set(value){if(!choices[value])return;current=value;apply();try{localStorage.setItem(key,current);}catch{}sync();}
 apply();sync();
 group.addEventListener('change',event=>{if(event.target.matches('[name="panelStyle"]')&&event.target.checked)set(event.target.value);});
 const api={sync,set,value:()=>current,reset:()=>set('sand')};window.spectralPanelStyle=api;return api;
}
