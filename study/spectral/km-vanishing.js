...
export function kmVanishingProof({math,t}){
 ...
 p(t(`只剩 ${math('i=0')} 一列。由 2.11，`,`Only column ${math('i=0')} remains. By 2.11,`))+
 eq(R`H^0(X,L(mH)\otimes R^jf_*\omega_Y)\cong H^j(Y,\omega_Y\otimes f^*L(mH))=0\qquad(j>0)`)+
 p(t(`充分大的扭曲 ${math(R`L(mH)\otimes R^jf_*\omega_Y`)} 由全局截面生成，故该层为零，从而 ${math(R`R^jf_*\omega_Y=0`)}（${math('j>0')}）。最后取 ${math('m=0')}，由 2.11 的单行情形得到`,`The sufficiently large twist ${math(R`L(mH)\otimes R^jf_*\omega_Y`)} is globally generated, hence zero. Thus ${math(R`R^jf_*\omega_Y=0`)} for ${math('j>0')}. Now set ${math('m=0')}; the single-row case of 2.15 yields`))+
