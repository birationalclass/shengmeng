...
export function operationMarkup(s,lang,math){
 ...
 if(s.module==='initial'&&!['total','totalmap'].includes(c)){
  ...
  if(s.step===0){body=a===1?row(box(R`H^n(C^\bullet,D)`,'chosen')):a===3?nested(R`H^n`,R`F^pH^n`,R`F^{p+1}H^n`,R`\operatorname{Gr}_F^pH^n`,'quotient'):row(box(R`a\in F^pC^n\cap\ker D`)+arrow(R`a\mapsto[a]_H`)+box(R`F^pH^n\subseteq H^n`,'chosen'));note=t('在总上同调中模掉所有总边界，包括来自更低滤过的边界。','Quotient by all total boundaries, including those from lower filtration.');}
  if(s.step===1){body=row(box(`a\\in F^{${p}}C^{${n}}`)+arrow('D')+box(`Da\\in F^{${p+r}}C^{${n+1}}${p+r>n+1?'=0':''}`,'chosen'))+`<div class="operation-equation">${M(p+r>n+1?R`Z_r^{p,q}=F^pC^n\cap\ker D`:R`p+r>n+1\ \Longrightarrow\ Z_r^{p,q}=F^pC^n\cap\ker D`)}</div>`;if(a===2)body=row(box(R`a\in Z_r^{p,q}`)+arrow('D')+box(p+r>n+1?'Da=0':R`Da\in F^{p+r}C^{n+1}`,'chosen'));if(a===3)body=row(box(`b\\in F^{${p-r}}C^{${n-1}}${r>=p?`=C^{${n-1}}`:''}`)+arrow('D')+box(R`Db\in F^pC^n`,'chosen'));note=t('目标滤过为零时，Da=0；r≥p 时，Bᵣ 包含 FᵖCⁿ 中的全部总边界。','A zero target filtration forces Da=0. For r≥p, Bᵣ contains all total boundaries in FᵖCⁿ.');}
  ...
  if(s.step===3){body=row(box(R`a=b+Dc`)+arrow(R`a\mapsto[a]_H+F^{p+1}H^n`)+box('0','chosen'))+`<div class="operation-equation">${M(R`b\in Z_\infty^{p+1,q-1},\quad Dc\in B_\infty^{p,q}`)}</div>`;if(a===1)body=row(box(R`[a]_H\in F^{p+1}H^n`)+arrow(R`\text{in Gr}`)+box('0','chosen'));if(a===3)body=row(box(R`Z_\infty^{p,q}`)+arrow(R`a\mapsto[a]_H+F^{p+1}H^n`)+box(R`\operatorname{Gr}_F^pH^n`))+`<div class="operation-equation">${M(R`\ker=Z_\infty^{p+1,q-1}+B_\infty^{p,q}`)}</div>`;if(a===4)body=nested(R`F^pC^n`,R`Z_\infty^{p,q}`,R`Z_\infty^{p+1,q-1}+B_\infty^{p,q}`,R`E_\infty^{p,q}`,'quotient');if(a===5)body=row(box(R`E_\infty^{p,q}`)+arrow(R`\theta^{p,q}\;\sim`)+box(R`\operatorname{Gr}_F^pH^n`,'chosen'));note=t('核恰为 E∞ 的分母；满射取商得到同构。','The kernel is exactly the denominator of E∞; the surjection induces an isomorphism.');}
