...
export function createSquareTrace(host){
 ...
 const play=(effect,centres)=>{
  ...
  layer.innerHTML=`<defs><linearGradient id="element-sweep" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="${color}" stop-opacity="0"/><stop offset=".5" stop-color="${color}" stop-opacity=".4"/><stop offset=".8" stop-color="${color}"/><stop offset="1" stop-color="${color}" stop-opacity="0"/></linearGradient></defs><g class="element-flow" fill="none" stroke="url(#element-sweep)" stroke-width="2.8" stroke-linecap="round">${centres.slice(0,2).map(([x,y],i)=>`<path d="M${x+ux*pad},${y+uy*pad} L${centres[i+1][0]-ux*pad},${centres[i+1][1]-uy*pad}"/>`).join('')}</g><g class="element-token"><circle class="element-dot" r="7" fill="${color}" stroke="#fff9" stroke-width=".7"/><text class="element-zero" x="0" y="1" text-anchor="middle" dominant-baseline="central" fill="${color}" stroke="#10252e" stroke-width="4" paint-order="stroke" font-family="KaTeX_Main,Georgia,serif" font-size="27">0</text></g>`;
  host.append(layer);
