...
export function createDifferentialSweep({host,point,math}){
 ...
 // Keep the mathematical glyphs and their metrics intact; crossfade only the index.
 function pageDigits(el){
  ...
  return [...slot.querySelectorAll('.dr-page-digit')];
 }
 function indexSemantics(el,r,tex){
  el.querySelector('.katex-mathml msubsup > mn').textContent=r;
  el.querySelector('.katex-mathml annotation').textContent=tex;
 }
 async function play({signal,wait}){
  ...
  settle=()=>{const r=Number(layer.dataset.r)||0,from=position,start=performance.now(),retained=layer;page.innerHTML=math(`r=${r}`);indexSemantics(arrowLabel,r,`d_${r}^{0,3}`);const finish=now=>{if(layer!==retained)return;const t=reduced()?1:Math.min(1,(now-start)/220);geometry(from+(r-from)*t*t*(3-2*t));if(t<1)requestAnimationFrame(finish);};requestAnimationFrame(finish);};
  ...
  for(let r=1;r<=5;r++){
   ...
   indexSemantics(arrowLabel,r,`d_${r}^{0,3}`);
   const labels=Promise.all([change(page,`r=${r}`),animate(arrowDigits[r-1],[{opacity:1},{opacity:0}],420),animate(arrowDigits[r],[{opacity:0},{opacity:1}],420),...digitChanges]);
