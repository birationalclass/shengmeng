...
export function createDifferentialSweep({host,point,math}){
 ...
 async function play({signal,wait}){
  ...
  const cells=[];for(let p=0;p<=4;p++)for(let q=0;q<=4;q++){
   const [x,y]=point(p,q);const el=text(x,y,`E_0^{${p},${q}}`,'dr-sweep-cell');el.dataset.p=p;el.dataset.q=q;cells.push({p,q,el});
  }
  ...
  geometry(0);highlight(0);host.append(layer);
  ...
  for(let r=1;r<=5;r++){
   ...
   const oldLabels=[];for(const {p,q,el}of cells){const old=document.createElement('span');old.className='dr-sweep-old';old.innerHTML=el.innerHTML;el.innerHTML=math(`E_${r}^{${p},${q}}`);el.append(old);oldLabels.push(old);}
   ...
   const labels=Promise.all([change(page,`r=${r}`),change(arrowLabel,`d_${r}^{0,3}`),...oldLabels.map(el=>animate(el,[{opacity:1},{opacity:0}],420).then(()=>el.remove()))]);
