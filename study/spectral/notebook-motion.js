...
export function createNotebookMotion({language}) {
 ...
 function sync() {
  ...
  window.spectralThemeCursor?.sync(language());
 ...
 $('#motionReset').onclick=()=>{effect.value=defaults.effect;duration.value=defaults.duration;autoCollapse.checked=defaults.autoCollapse;window.spectralCover?.setStyle('c');window.spectralPanelStyle?.reset();window.spectralThemeCursor?.reset();update();};
