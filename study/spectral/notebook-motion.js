...
export function createNotebookMotion({language}) {
 ...
 function setExpanded(body,open,{immediate=false}={}) {
  ...
  if(previous?.animation){previous.animation.onfinish=null;previous.animation.cancel();running.delete(body);}
  const record={open,animation:null};records.set(body,record);
  body.hidden=false;body.inert=!open;body.classList.remove('accordion-moving');
  const natural=body.scrollHeight,to=open?natural:0,ms=milliseconds();
  const finish=()=>{if(records.get(body)!==record)return;body.hidden=!open;body.classList.remove('accordion-moving');if(record.animation){record.animation.onfinish=null;record.animation.cancel();record.animation=null;}running.delete(body);};
