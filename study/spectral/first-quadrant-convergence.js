...
export function firstQuadrantContent({math,t}){
 ...
 para(t(`<strong>关于收敛定义的约定。</strong> McCleary 定义 2.4 只要求目标上存在某个滤过，使稳定项同构于其关联分次。本文 2.8 则固定 1.12 的诱导滤过，并要求上述由闭代表元给出的自然同构；这是一项针对本文双复形构造的具体约定，不应视为 McCleary 定义 2.4 的原文，也不另称为“强收敛”。`,`<strong>Convention on convergence.</strong> McCleary’s Definition 2.4 requires the existence of a filtration on the target whose associated graded is isomorphic to the limit term. Our 2.8 fixes the induced filtration from 1.12 and requires the natural isomorphism on cocycle representatives above. This is a convention for our double-complex construction, not the literal statement of Definition 2.4, and we do not introduce the term “strong convergence” for it.`))+
