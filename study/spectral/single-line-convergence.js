...
export function singleLineContent({math,t}){
 ...
 para(t(`对 ${math('1\\le p\\le n')}，商空间为零意味着 ${math('F^pH^n=F^{p+1}H^n')}。再利用命题假设中有限滤过的端点 ${math('F^{n+1}H^n=0')}，从后向前递推得到 ${math('F^pH^n=0\\ (1\\le p\\le n+1)')}。这里端点为零是必需的；仅有各商为零只能推出相邻滤过项相等。`, `For ${math('1\\le p\\le n')}, the zero quotient implies ${math('F^pH^n=F^{p+1}H^n')}. Starting from the endpoint ${math('F^{n+1}H^n=0')} of the finite filtration assumed in the proposition, descending induction gives ${math('F^pH^n=0\\ (1\\le p\\le n+1)')}. The zero endpoint is essential: vanishing quotients alone only give equality of adjacent filtration terms.`))+
