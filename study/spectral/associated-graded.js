...
export const gradedFormulas=[
 ...
 R`\begin{gathered}\operatorname{Gr}_F^pD^n:\\[-.2em]\begin{array}{rcl}\operatorname{Gr}_F^pC^n&\longrightarrow&\operatorname{Gr}_F^pC^{n+1}\\[.25em][a]&\longmapsto&[Da]\end{array}\end{gathered}`
];
export function createGradedProof({board,math,language}){
 let step=0,context=null;const t=(zh,en)=>language()==='en'?en:zh;
 const entries=[
  {name:['空间与元素','Spaces and elements'],f:[R`\pi_p^n:F^pC^n\longrightarrow\operatorname{Gr}_F^pC^n`,R`[a]:=a+F^{p+1}C^n,\quad a\in F^pC^n`,R`\pi_p^n(a)=[a],\qquad\ker\pi_p^n=F^{p+1}C^n`],note:['关联分次取出相邻滤过层之间的商。两个代表元给出同一类，当且仅当其差属于下一层滤过。','The associated graded takes quotients of consecutive filtration layers. Two representatives define the same class exactly when their difference lies in the next layer.']},
  {name:['诱导微分','Induced differential'],f:[R`[a]:=a+F^{p+1}C^n,\quad a\in F^pC^n`,R`(\operatorname{Gr}_F^pD^n)([a]):=[Da]=Da+F^{p+1}C^{n+1}`,R`a\in K^{p,n-p}\quad\Longrightarrow\quad[Da]=[\delta_1a]+[\delta_2a]=[\delta_2a]`,R`\delta_1a\in F^{p+1}C^{n+1}\quad\Longrightarrow\quad[\delta_1a]=[0]`,R`D(F^{p+1}C^n)\subseteq F^{p+1}C^{n+1}`,R`\begin{aligned}a'-a\in F^{p+1}C^n\quad&\Longrightarrow\\Da'-Da\in F^{p+1}C^{n+1}.&\end{aligned}`,R`\operatorname{Gr}_F^pD^n\circ\pi_p^n=\pi_p^{n+1}\circ D|_{F^pC^n}`],note:['D 保持滤过，所以它的像仍在目标滤过层；它也保持下一层，因此商上的像不依赖代表元。Gr 不只是空间的记号，也作用于保持滤过的映射。','D preserves the filtration, so its image belongs to the target layer. It also preserves the next layer, making the quotient map independent of representatives. Gr acts on filtration-preserving maps as well as spaces.']},
  ...
  {name:['与双复形的关系','Relation to the double complex'],f:[R`\operatorname{Gr}_F^pC^n\cong K^{p,n-p}`,R`\begin{array}{rcl}\Phi^{p,n}:K^{p,n-p}&\xrightarrow{\sim}&\operatorname{Gr}_F^pC^n\\a&\longmapsto&[a]\end{array}`,R`(\operatorname{Gr}_F^pD^n)\Phi^{p,n}=\Phi^{p,n+1}\delta_2^{p,n-p}`,R`\delta_1a\in F^{p+1}C^{n+1}`],note:[R`取商后，横向分量的类为零，纵向分量保留。这是同构下的对应，并非总复形上 $D=\delta_2$。第二节从各页统一的商公式定义 $E_0^{p,q}$，再证明它等于这里的关联分次，且 $d_0^{p,q}$ 等于这里的诱导微分。`,R`The horizontal component vanishes in the quotient; the vertical component remains. This identification does not assert $D=\delta_2$ on the total complex. Section 2 defines $E_0^{p,q}$ by the quotient formula shared by all pages, then identifies it and $d_0^{p,q}$ with this associated graded and its induced differential.`]}
 ];
 ...
 return {render(state){if(context?.effect!==state.effect&&state.effect==='gradedmap')step=1;context={...state};paint();},reset(){context=null;step=0;}};
}
