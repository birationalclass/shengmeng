...
const R=String.raw;
export function pageStackMarkup({start=0,current=0,p=1,q=2,lang='zh'},math){
 const t=(zh,en)=>lang==='en'?en:zh,colors=['#8fbeff','#71e2d0','#f4c876','#d9b7ec'];
 const point=(i,j,slot)=>[120+68*i+40*j,456+10*i-15*j-110*slot];
 ...
 let out=`<svg viewBox="0 0 840 525" role="group" aria-label="${t('谱序列各页：p、q、r 三维坐标','Spectral-sequence pages in p, q, r coordinates')}"><defs>`;
 ...
 out+=`<path class="stack-axis" d="M120,456 V24" marker-end="url(#stack-tip-0)"/><text class="stack-axis-name" x="109" y="18">r</text>`;
 if(start>0)out+=`<text class="stack-axis-tick" x="109" y="495">⋮</text>`;
 out+=`<text class="stack-axis-tick" x="110" y="58">⋮</text>`;
 for(let slot=0;slot<4;slot++){
  ...
  out+=`<g class="stack-layer-tab" role="button" tabindex="0" data-stack-r="${r}" aria-pressed="${active}" aria-label="${t('选择第','Select page ')} ${r} ${t('页','')}"><rect x="620" y="${origin[1]-40}" width="155" height="55" rx="9"/>${symbol(640,origin[1]-7,r)}<text class="stack-tab-hint" x="699" y="${origin[1]-9}">${active?t('当前','Selected'):t('查看','Inspect')}</text></g>`;
  out+=`<path class="stack-tick" d="M116,${origin[1]} h8"/><text class="stack-axis-tick" x="101" y="${origin[1]+5}" text-anchor="end">${r}</text></g>`;
 }
 ...
 for(let i=0;i<=4;i++){let a=point(i,0,0),b=point(0,i,0);out+=`<text class="stack-axis-tick" x="${a[0]}" y="${a[1]+20}" text-anchor="middle">${i}</text>`;if(i)out+=`<text class="stack-axis-tick" x="${b[0]-12}" y="${b[1]-4}">${i}</text>`;}
 out+='</svg>';
