import {foundations} from './simplicial-foundations.js?v=10';
...
model.no='2.1';model.title=pair('复代数簇的奇异上链','Singular cochains of complex varieties');
model.intro=pair(R`取复代数簇族 $X_p$ 及面映射 $d_i^{(p+1)}:X_{p+1}\to X_p$，满足第一节所述的单纯关系。$X_p^{\mathrm{an}}$ 表示其解析空间。`,R`Take complex algebraic varieties $X_p$ and face maps $d_i^{(p+1)}:X_{p+1}\to X_p$ satisfying the simplicial identities of Section 1. Write $X_p^{\mathrm{an}}$ for the associated analytic space.`);
model.formulas=[R`K^{p,q}:=C^q_{\mathrm{sing}}(X_p^{\mathrm{an}},\mathbb Q)`,R`\begin{gathered}C^q_{\mathrm{sing}}(X_p^{\mathrm{an}},\mathbb Q)\\=\operatorname{Hom}_{\mathbb Q}(C_q^{\mathrm{sing}}(X_p^{\mathrm{an}},\mathbb Q),\mathbb Q)\end{gathered}`,R`K^{p,q}=0\quad(p<0\ \text{or}\ q<0)`];
model.tag=pair('此构造不要求光滑或射影','This construction requires neither smoothness nor projectivity');
model.proof=[step('奇异上链空间','The singular cochain space',[R`d_{\mathrm{sing}}a=a\circ\partial_{\mathrm{sing}}`],pair(R`$C_q^{\mathrm{sing}}$ 是由连续奇异 $q$-单形生成的有理链空间；其对偶为这里的 $K^{p,q}$。奇异上链对任意拓扑空间都有定义；面映射的连续性和单纯关系足以构造该双复形。第一节已给出族 $X_\bullet$ 的完整定义。`,R`The rational singular chain space $C_q^{\mathrm{sing}}$ is generated by continuous singular $q$-simplices. Its dual is $K^{p,q}$. Singular cochains are defined for every topological space; continuity of the face maps and the simplicial identities suffice for this double-complex construction. Section 1 gives the full definition of $X_\bullet$.`))];
model.refs=[{course:'mhs',entry:'1.2',label:pair('单纯对象的完整定义','Full definition of the simplicial object')},{course:'spectral',entry:'1.1',label:pair('一般双复形：谱序列 §1','General double complexes: Spectral §1')}];
const differentials={id:'differentials',no:'2.2',kind:'definition',source:'MHS_EQUIV',page:1,graph:'complex',title:pair('面拉回与带符号的微分','Face pullbacks and signed differentials'),formulas:[R`\delta_1^{p,q}:=\sum_{i=0}^{p+1}(-1)^i(d_i^{(p+1)})^*:K^{p,q}\longrightarrow K^{p+1,q}`,R`\delta_2^{p,q}:=(-1)^p d_{\mathrm{sing}}:K^{p,q}\longrightarrow K^{p,q+1}`],proof:modelProof.slice(0,2),refs:[{course:'spectral',entry:'1.5',label:pair('反交换约定','Anticommuting convention')}]};
const total={id:'total-complex',no:'2.3',kind:'definition',source:'MHS_EQUIV',page:1,graph:'filtration',title:pair('本文的总复形与列滤过','Our total complex and column filtration'),intro:pair('对上述具体双复形沿用谱序列课件的构造与记号。','Apply the constructions and notation of the spectral-sequence notes to this specific double complex.'),formulas:[R`C^n:=\bigoplus_{p+q=n}C^q_{\mathrm{sing}}(X_p^{\mathrm{an}},\mathbb Q),\qquad D:=\delta_1+\delta_2`,R`F^pC^n:=\bigoplus_{i\ge p}K^{i,n-i}`,R`F^pH_C^n:=\operatorname{im}\bigl(H^n(F^pC^\bullet,D)\to H^n(C^\bullet,D)\bigr)`],proof:[step('与谱序列课件一致的滤过','The same filtration as in the spectral notes',[R`H_C^n:=H^n(C^\bullet,D)`],pair('这里固定的是列滤过及其自然诱导的上同调滤过。一般构造见下方引用；第三节将它传递到单纯上同调并记为 L。','We fix the column filtration and its induced cohomology filtration. The general construction is linked below. Section 3 transports it to simplicial cohomology and denotes it by L.'))],refs:[{course:'spectral',entry:'1.9',part:2,label:pair('微分保持滤过','The differential preserves the filtration')},{course:'spectral',entry:'1.12',label:pair('诱导上同调滤过','Induced cohomology filtration')}]};
const transported=by('filtrations');transported.no='3.1';transported.title=pair('单纯上同调与滤过 L','Simplicial cohomology and its filtration L');
...
transported.proof=[step('传递既定滤过','Transport the specified filtration',[R`[a]_D\in L^pH_X^n\ \Longleftrightarrow\ \exists b\in F^pC^n:\ Db=0,\ [b]_D=[a]_D`],pair(R`按 $H_X^n=H_C^n$ 把第二节的诱导滤过 $F$ 记为 $L$。它不是另行选取的滤过；此处也没有引入额外的簇 $X$ 或断言上同调下降。`,R`Under $H_X^n=H_C^n$, write the induced filtration $F$ of Section 2 as $L$. It is not an independently chosen filtration. No additional variety $X$ or descent identification is asserted here.`))];
const hodge=by('hodge');hodge.no='3.2';
...
by('goal').no='4.1';by('action').no='4.2';by('pages').no='4.3';by('convergence').no='4.4';by('degeneration').no='4.5';by('equivariance').no='4.6';by('representative').no='4.7';by('example').no='5.1';by('descent').no='5.2';
...
const ordered=[...foundations,model,differentials,total,transported,hodge,by('goal'),by('action'),by('pages'),by('convergence'),by('degeneration'),by('equivariance'),by('representative'),by('example'),by('descent')];
