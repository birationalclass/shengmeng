...
model.intro=pair(R`取光滑射影复簇族 $X_p$ 及面映射 $d_i^{(p+1)}:X_{p+1}\to X_p$，满足第二节所述的单纯关系。$X_p^{\mathrm{an}}$ 表示其解析空间。`,R`Take smooth projective complex varieties $X_p$ and face maps $d_i^{(p+1)}:X_{p+1}\to X_p$ satisfying the simplicial identities of Section 2. Write $X_p^{\mathrm{an}}$ for the associated analytic space.`);
...
model.tag=pair('允许每层为有限个光滑射影复簇的不交并','Each level may be a finite disjoint union of smooth projective complex varieties');
model.proof=[step('奇异上链空间','The singular cochain space',[R`d_{\mathrm{sing}}a=a\circ\partial_{\mathrm{sing}}`],pair(R`$C_q^{\mathrm{sing}}$ 是由连续奇异 $q$-单形生成的有理链空间；其对偶为这里的 $K^{p,q}$。第二节给出族 $X_\bullet$ 的完整定义。`,R`The rational singular chain space $C_q^{\mathrm{sing}}$ is generated by continuous singular $q$-simplices. Its dual is $K^{p,q}$. Section 2 gives the full definition of $X_\bullet$.`))];
...
simplicial.intro=pair(R`设 $X_\bullet:\Delta^{\mathrm{op}}\to\mathrm{Sch}_{\mathbb C}$ 为单纯概形，每个 $X_p$ 是有限个光滑射影复簇的不交并。给定单纯自映射 $f_\bullet$，其各层分量记为 $f_p$。`,R`Let $X_\bullet:\Delta^{\mathrm{op}}\to\mathrm{Sch}_{\mathbb C}$ be a simplicial scheme, each $X_p$ a finite disjoint union of smooth projective complex varieties. Let $f_\bullet$ be a simplicial endomorphism with components $f_p$.`);
...
hodge.intro=pair(R`置 $V=H_X^n$。$W$ 是有理空间上的权重滤过，$F_{\mathrm{Hdg}}$ 是复化上的 Hodge 滤过；混合 Hodge 结构简称 MHS。`,R`Put $V=H_X^n$. The weight filtration $W$ is rational, and the Hodge filtration $F_{\mathrm{Hdg}}$ is on the complexification. MHS abbreviates mixed Hodge structure.`);
