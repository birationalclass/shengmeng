...
import {smooth,clamp,mix,cameraAt,SITES} from './story.mjs?v=20260920h';
const TAU=Math.PI*2;
...
export class AtlasScene{
 constructor(canvas,quality='standard'){
  this.canvas=canvas;this.renderer=new T.WebGLRenderer({canvas,antialias:true,alpha:false,powerPreference:'high-performance'});this.renderer.setClearColor(0x0a1511);this.renderer.outputColorSpace=T.SRGBColorSpace;this.renderer.toneMapping=T.ACESFilmicToneMapping;this.renderer.toneMappingExposure=.98;this.renderer.shadowMap.enabled=true;this.renderer.shadowMap.type=T.PCFSoftShadowMap;
  this.scene=new T.Scene();this.scene.background=new T.Color(0x667e82);this.scene.fog=new T.FogExp2(0x667e82,.0035);this.camera=new T.PerspectiveCamera(42,1,.1,350);this.rotating=[];this.risers=[];this.manual=null;
  ...
  this.scene.add(new T.HemisphereLight(0xfce8bb,0x36454b,1.3));const key=new T.DirectionalLight(0xffe6bd,3.3);key.position.set(-13,26,18);key.castShadow=true;key.shadow.mapSize.set(2048,2048);Object.assign(key.shadow.camera,{left:-65,right:65,top:65,bottom:-65,near:1,far:180});key.shadow.bias=-.0005;key.shadow.normalBias=.045;this.scene.add(key);this.key=key;const fill=new T.DirectionalLight(0xb1c6ce,1.4);fill.position.set(6,11,-22);this.scene.add(fill);const rim=new T.DirectionalLight(0xfff0d1,1.1);rim.position.set(25,13,5);this.scene.add(rim);
  this.makeTerrain();
  this.platforms=SITES.map(([x,y,z],i)=>{const p=this.platform(x,i);p.position.z=z;return p;});
  this.makeWriting(this.platforms[0]);this.makePagoda(this.platforms[1]);this.makeNewton(this.platforms[2]);this.makePermutation(this.platforms[3]);this.makeCrystal(this.platforms[4]);this.makeAI(this.platforms[5]);this.makeConnections();this.makeDust();this.makeOrrery();this.setQuality(quality);this.resize();
 }
 ...
 platform(x,i){const p=new T.Group();p.position.x=x;this.scene.add(p);this.cylinder(p,7.65,.6,[0,-.55,0],this.materials.dark);this.cylinder(p,7.38,.23,[0,-.16,0],this.materials.brass);this.cylinder(p,7.08,.27,[0,.04,0],this.materials.stone);this.torus(p,7.22,.08,[0,.15,0]);this.torus(p,6.78,.035,[0,.20,0]);
 ...
 makeNameplate(p,i){
  ...
  const face=this.mesh(plaque,new T.PlaneGeometry(11.45,2.13),new T.MeshStandardMaterial({map:texture(c),roughness:.82,metalness:.05}),[0,.262,0]);face.rotation.x=-Math.PI/2;
 ...
 makeChineseHouses(p){
  ...
  const hall=new T.Group();group.add(hall);hall.position.set(2.2,.3,-1.1);hall.rotation.y=-.15;this.box(hall,[3.0,1.6,1.75],[0,.83,0],wall);this.box(hall,[3.3,.18,2.05],[0,.02,0],this.materials.stone);
  ...
  const pavilion=new T.Group();group.add(pavilion);pavilion.position.set(2.65,.2,3.15);this.cylinder(pavilion,1.3,.2,[0,.18,0],this.materials.stone);for(let j=0;j<6;j++){const a=j*TAU/6;this.cylinder(pavilion,.075,1.7,[Math.sin(a)*.91,1.10,Math.cos(a)*.91],wood);}this.mesh(pavilion,new T.ConeGeometry(1.53,.72,6),this.materials.dark,[0,2.2,0]);this.mesh(pavilion,new T.SphereGeometry(.09,10,8),this.materials.gold,[0,2.64,0]);
 ...
 makeCathedral(g){
  const church=new T.Group();g.add(church);church.position.set(0,.82,-.85);const stone=this.materials.stone,roof=this.materials.dark;
 ...
 makePagoda(p){
  ...
  for(let floor=0;floor<5;floor++){const level=new T.Group(),w=3.5-floor*.49,y=.85+floor*1.67;g.add(level);this.pagodaFloors.push({level,y,start:41+floor*2.8,end:44+floor*2.8});
   ...
   for(let side=0;side<4;side++){const angle=side*Math.PI/2;for(let j=-6;j<=6;j++){const q=j/6,points=[];for(let k=0;k<=6;k++){const u=k/6,extent=w*1.12*(1-u*.74),x=q*extent,z=extent;points.push([x*Math.cos(angle)-z*Math.sin(angle),1.235+.53*u*u+.16*(1-u)**5+.09*Math.abs(q)**6*(1-u),x*Math.sin(angle)+z*Math.cos(angle)]);}this.line(roofRoot,points,0x998568,.43);}}
  }
 ...
 makeCrystal(p){
  const g=new T.Group();p.add(g);this.crystal=g;this.risers.push({object:g,start:99,end:107});this.gear(g,3.3,48,[0,.4,0],-.10);
  const mat=new T.MeshPhysicalMaterial({color:0xb5cfca,metalness:.3,roughness:.2,clearcoat:1,clearcoatRoughness:.14,flatShading:true});this.crystalInstances=new T.InstancedMesh(new T.OctahedronGeometry(.53),mat,27);g.add(this.crystalInstances);this.crystalNodes=[];
 ...
 animateNewScenes(t){
  ...
  for(const {g,start,end}of this.architecture){const amount=smooth((t-start)/(end-start));g.scale.y=Math.max(.01,amount);g.visible=amount>.001;}
 ...
 update(t){
  ...
  for(const {object,start,end}of this.risers){const amount=smooth((t-start)/(end-start));object.position.y=-1.5*(1-amount);object.scale.y=Math.max(.02,amount);object.visible=amount>.002;}
