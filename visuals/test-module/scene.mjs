...
export class AtlasScene{
 constructor(canvas,quality='standard',options={}){
  this.canvas=canvas;this.legacyScenery=options.legacyScenery!==false;this.renderer=new T.WebGLRenderer({canvas,antialias:options.antialias!==false,alpha:false,powerPreference:options.powerPreference||'high-performance'});this.renderer.setClearColor(0x0a1511);this.renderer.outputColorSpace=T.SRGBColorSpace;this.renderer.toneMapping=T.ACESFilmicToneMapping;this.renderer.toneMappingExposure=.94;this.renderer.shadowMap.enabled=true;this.renderer.shadowMap.type=T.PCFSoftShadowMap;
  ...
  if(this.legacyScenery){this.makeWriting(this.platforms[0]);this.makePagoda(this.platforms[1]);this.makeNewton(this.platforms[2]);this.makePermutation(this.platforms[3]);this.makeCrystal(this.platforms[4]);this.makeAI(this.platforms[5]);}
  this.makeConnections();if(this.legacyScenery)refineArchitecture(this);this.makeDust();if(this.legacyScenery)this.makeOrrery();else this.orrery=new T.Group();this.setQuality(quality);this.resize();
 }
