import {AtlasScene} from './scene.mjs?v=20260920b';
import {DURATION,PHASES,phaseAt,clamp} from './story.mjs?v=20260920b';
const $=id=>document.getElementById(id),canvas=$('world');
const defaults={speed:1,quality:'standard',captions:true,dust:true,loop:false,reduced:matchMedia('(prefers-reduced-motion: reduce)').matches};
let prefs={...defaults};try{const saved=JSON.parse(localStorage.getItem('operations-atlas-v1')||'{}');for(const k of Object.keys(defaults))if(typeof saved[k]===typeof defaults[k])prefs[k]=saved[k];}catch{}
if(!['standard','high','low'].includes(prefs.quality))prefs.quality='standard';if(![.65,1,1.5].includes(prefs.speed))prefs.speed=1;
let scene,t=prefs.reduced?18:0,playing=!prefs.reduced,last=0,phase=null,captionTimer,frame=0,failed=false,dirty=true;
const save=()=>{try{localStorage.setItem('operations-atlas-v1',JSON.stringify(prefs));}catch{}};
const clock=n=>`${String(Math.floor(n/60)).padStart(2,'0')}:${String(Math.floor(n%60)).padStart(2,'0')}`;
function setPlaying(value){playing=value&&!failed;$('play').textContent=playing?'Ⅱ':'▷';$('play').setAttribute('aria-label',playing?'暂停动画':'播放动画');$('cinema').dataset.playing=String(playing);last=0;}
function caption(p){$('era').textContent=p.era;$('englishTitle').textContent=p.en;$('chineseTitle').textContent=p.zh;$('narration').textContent=p.line;$('englishNarration').textContent=p.sub;$('formula').textContent=p.formula;const n=PHASES.indexOf(p);$('sceneNumber').textContent=n===0?'PROLOGUE':n===4?'CODA':`CHAPTER 0${n}`;const actions={semigroup:'让字母相连 ↗',action:'看苹果下落 ↗',composition:'沿着计算前进 ↗',epilogue:'重看序曲 ↗'};$('sceneAction').hidden=!actions[p.id];$('sceneAction').textContent=actions[p.id]||'';document.querySelector('.titles').classList.remove('changing');}
...
function jump(value,play=false){t=clamp(value,0,DURATION);automatic();sync(true);dirty=true;if(play)setPlaying(true);}
...
$('play').onclick=()=>{if(t>=DURATION)jump(0);setPlaying(!playing);};$('restart').onclick=()=>jump(0,true);$('seek').oninput=e=>jump(Number(e.target.value));
for(const b of document.querySelectorAll('[data-time]'))b.onclick=()=>jump(Number(b.dataset.time),!prefs.reduced);
$('sceneAction').onclick=()=>jump({semigroup:23,action:50,composition:80,epilogue:0}[phaseAt(t).id]??0,true);
...
function applyPrefs(){for(const key of Object.keys(defaults)){const el=$(key);if(el.type==='checkbox')el.checked=prefs[key];else el.value=prefs[key];}document.body.classList.toggle('no-captions',!prefs.captions);if(scene){scene.dust.visible=prefs.dust;scene.setQuality(prefs.quality);}dirty=true;}
...
document.addEventListener('keydown',e=>{if(document.querySelector('dialog[open]')||/INPUT|SELECT|TEXTAREA/.test(e.target.tagName))return;if(e.code==='Space'&&e.target.tagName!=='BUTTON'){e.preventDefault();$('play').click();}else if(e.code==='ArrowRight'){e.preventDefault();jump(t+5);}else if(e.code==='ArrowLeft'){e.preventDefault();jump(t-5);}else if(['1','2','3'].includes(e.key)){jump([8,38,70][Number(e.key)-1],!prefs.reduced);}});
window.addEventListener('resize',()=>{scene?.resize();dirty=true;});document.addEventListener('visibilitychange',()=>last=0);
...
function animate(now){if(failed)return;requestAnimationFrame(animate);if(document.hidden){last=0;return;}const dt=last?Math.min((now-last)/1000,.15):0;last=now;
 if(playing){t+=dt*prefs.speed;if(t>=DURATION){if(prefs.loop)t%=DURATION;else{t=DURATION;setPlaying(false);}}sync();}
 if(playing||dirty){scene.update(t);dirty=false;frame++;$('cinema').dataset.frames=frame;}
}
applyPrefs();sync(true);setPlaying(playing);
try{await Promise.race([document.fonts.ready,new Promise(resolve=>setTimeout(resolve,2000))]);scene=new AtlasScene(canvas,prefs.quality);applyPrefs();scene.update(t);$('cinema').dataset.ready='true';$('loadStatus').textContent='地图已就绪';requestAnimationFrame(()=>{requestAnimationFrame(()=>{$('loading').hidden=true;requestAnimationFrame(animate);});});}catch(error){fail(error);}
