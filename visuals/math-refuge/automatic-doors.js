...
export function createAutomaticDoors(T,glass,metal){
  ...
  function add(parent,{x,y,z,width,height,axis,name}){
    ...
    const leaves=[],seams=[];
    const seamMaterial=name==='west'?metal.clone():null;
    ...
    for(const sign of [-1,1]){
      ...
      for(const edge of [-1,1]){
        const jamb=new T.Mesh(geometry,seamMaterial||metal);jamb.name='Sliding door seam';if(seamMaterial){jamb.visible=false;seams.push(jamb);}jamb.scale.set(.013,height,.032);jamb.position.x=edge*width/4;leaf.add(jamb);
      }
