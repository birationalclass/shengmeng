import {createFontAnimator} from './font-animation.js?v1';
import {inkGuides,writingPlan,writingPose,inkReveal} from './chalk-motion.js?v49-late-frames';
// Authored centreline strokes. These are not outlines extracted from a font.
import {glyphs,advance} from './handwriting-glyphs.js?v66-galaxy';
...
function add(points,size){strokes.push({points,width:Math.max(1.45,size*.05),row,seed:id++});}
function text(value,x,y,size=40){const op={value,x,y,size,row};textOps.push(op);for(const char of value){if(char!==' '){if(!library.has(char))throw Error('Missing authored strokes: '+char);for(const path of library.get(char))add(path.map(([a,b])=>[x+a*size/75,y+(b-72)*size/75]),size);}x+=(advance[char]||65)*size/75;}op.width=x-op.x;return x;}
function fraction(top,bottom,x,y,size=36){const width=s=>[...s].reduce((w,c)=>w+(advance[c]||65)*size/75,0);const w=Math.max(width(top),width(bottom))+18;text(top,x+(w-width(top))/2,y-15,size);add(Array.from({length:Math.ceil(w/2)+1},(_,i)=>{const t=i/Math.ceil(w/2);return[x+w*t,y+Math.sin(t*Math.PI*2+.5)*.55];}),size);strokes.at(-1).bar=true;text(bottom,x+(w-width(bottom))/2,y+size+16,size);return x+w+15;}
...
function fontSample(){
 ctx.clearRect(0,0,1536,640);const family=fontChoice.value;let r=0;
 ctx.font=`100px ${family}`;const cap=ctx.measureText('H').actualBoundingBoxAscent||70;
 function font(size){ctx.font=`${size*.85*100/cap}px ${family}`;}
 function write(value,x,y,size=40){font(size);ctx.fillStyle=r===6?'#e0cb9b':'#eee9d5';for(const part of value.split(/([<>≥≤⇔⇒=+−])/)){if(!part)continue;if(library.has(part)&&/^[<>≥≤⇔⇒=+−]$/.test(part)){for(const path of library.get(part))for(let i=1;i<path.length;i++)segment(ctx,[x+path[i-1][0]*size/95,y+(path[i-1][1]-72)*size/95],[x+path[i][0]*size/95,y+(path[i][1]-72)*size/95],Math.max(1.3,size*.038));x+=(advance[part]||65)*size/95;}else{ctx.fillText(part,x,y);x+=ctx.measureText(part).width;}}return x;}
 function frac(top,bottom,x,y,size=36){font(size);const tw=ctx.measureText(top).width,bw=ctx.measureText(bottom).width,w=Math.max(tw,bw)+18;write(top,x+(w-tw)/2,y-15,size);segment(ctx,[x,y],[x+w*.49,y-.6],1.6);segment(ctx,[x+w*.49,y-.6],[x+w,y+.2],1.6);write(bottom,x+(w-bw)/2,y+size+16,size);return x+w+15;}
 function line(fn){const k=r<4?formulaScale:1,base=[135,239,344,435][r]||0;ctx.save();ctx.translate(87,base);ctx.scale(k,k);ctx.translate(-87,-base);fn();ctx.restore();r++;}
 authorPage(write,frac,line);write('Proposition 3.2 · The threshold 243',84,62,30);
 tip.style.opacity='0';status.textContent='整页字形对比';
}
function render(time,seek=false){
 if(fontChoice.value!=='original'){fontSample();return;}
...
const fontAnimator=createFontAnimator({board,tip,progress,status,metrics,play,speed:document.querySelector('#speed'),paint:fontSample});
play.onclick=()=>{if(fontChoice.value!=='original'){fontAnimator.toggle();return;}if(t>=duration){t=0;reset();}playing=!playing;play.textContent=playing?'暂停':'播放';last=0;};
progress.oninput=()=>{if(fontChoice.value!=='original'){fontAnimator.seek(Number(progress.value)/1000);return;}playing=false;play.textContent='播放';t=Number(progress.value)/1000*duration;render(t,true);stats();};
fontChoice.onchange=async()=>{playing=false;play.textContent='播放';fontAnimator.stop();await document.fonts.load('40px '+fontChoice.value);const original=fontChoice.value==='original';mode.disabled=!original;mode.value='stroke';document.querySelector('#speed').disabled=false;if(original){play.disabled=false;progress.disabled=false;render(t,true);stats();}else await fontAnimator.prepare(true);};
let sizeTimer;sizeControl.oninput=()=>{formulaScale=Number(sizeControl.value)/100;document.querySelector('#formulaSizeValue').textContent=sizeControl.value+'%';clearTimeout(sizeTimer);if(fontChoice.value==='original')render(t,true);else{fontAnimator.stop();fontSample();sizeTimer=setTimeout(()=>fontAnimator.prepare(true),220);}};
mode.onchange=()=>{frames=[];render(t,true);stats();};document.addEventListener('visibilitychange',()=>last=0);window.addEventListener('resize',()=>fontChoice.value==='original'?render(t):fontAnimator.resize());
await Promise.all(['RefugeLatin','SampleCourgette','SampleEdu','SampleCaveat'].map(f=>document.fonts.load('40px '+f)));reset();render(0);stats();fontChoice.onchange();
const invalid=strokes.filter(s=>s.points.some(([x,y])=>!Number.isFinite(x+y)||x<0||x>1536||y<85||y>632));document.querySelector('#validation').textContent=`校验：${invalid.length===0?'全部轨迹位于书写区域内':'发现越界'}；每笔有独立抬笔段，回看时重建同一笔迹。`;board.dataset.validation=String(invalid.length);requestAnimationFrame(frame);
