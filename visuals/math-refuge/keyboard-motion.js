...
export class KeyboardMotion {
 ...
 reset(){this.held=0;this.signature='';this.velocity=[0,0,0];this.turn=0;}
 update(keys,dt){
  ...
  const length=Math.hypot(...input),signature=length?input.join(','):'';
  this.held=signature&&signature===this.signature?this.held+dt:0;this.signature=signature;
  const t=Math.min(1,Math.max(0,(this.held-2)/6)),ramp=t*t*(3-2*t);
