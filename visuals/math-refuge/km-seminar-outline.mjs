...
// Six substantive boards per original section; proof sketches are explicitly labelled.
const R=String.raw;
...
从曲面收缩提出问题|E\simeq\mathbf P^1,\quad E^2=-1,\quad K_X\cdot E=-1|在光滑曲面上，伴随公式把负自交有理曲线与典范类联系起来。Castelnuovo 收缩以光滑点为像；高维不能把此描述原样照搬。|The surface model|Adjunction gives K·E=-1 for a (-1)-curve. Its contraction stays smooth; higher dimensions require singularities.
变形的 Euler 特征|\chi(C,f^*T_X)=-K_X\cdot f_*C+n(1-g)|设 X 光滑且 n 为维数。变形理论给出 Hom 空间的局部维数下界；右端由向量丛 Riemann–Roch 计算，不能把下界误当等式。|Counting deformations|For smooth X, deformation theory bounds the local Hom dimension below by this Euler characteristic.
固定点与重新参数化|\dim_{[f]}\operatorname{Hom}(C,X;f(p))\ge -K_X\cdot f_*C-ng|固定一个像点至多施加 n 个条件。应用弯折断裂还须排除只改变参数而不改变像的族；有正维 Hom 并不自动给出移动曲线。|Fixing an image point|Subtract at most n conditions and exclude families arising only from automorphisms of the source.
弯折与断裂的机制|C\times T\dashrightarrow X,\qquad Z_0=Z'+\sum_i a_iR_i|证明提纲：完备化一参数族并解消有理映射，缺失纤维产生有理分量。应逐项说明族确实移动、固定点条件以及极限作为一循环的意义。|Bend and break: proof map|Compactify a moving family and resolve its map. Exceptional rational curves appear in the limiting cycle.
Frobenius 放大而不增加亏格|f\circ F_C^e,\qquad -K_X\cdot(f\circ F_C^e)_*C=p^e(-K_X\cdot f_*C)|证明提纲：约化到正特征后用 Frobenius 放大次数，源曲线亏格不变。最后必须使用有界次数和参数空间，把存在性传回特征零。|The Frobenius step|After reduction, Frobenius increases degree without increasing genus. Bounded degree is essential when returning to characteristic zero.
Mori 定理与边界检验|0<-K_X\cdot R\le n+1|定理 1.10：光滑射影 Fano 簇的每一点都在这样的有理曲线上。Mori (1982)。讨论：射影空间中的直线达到上界；为何证明需要光滑性？|Mori's bound|Mori (1982), Theorem 1.10: every point of a smooth projective Fano lies on such a rational curve.
`);
...
勘误四：逐项减去边界|f^*L=L'+E,\quad f^*H=H'+E;\qquad K_Y+L'+H'=f^*(K_X+L+H)-E|右边 E 的系数来自一减一减一，故 a(E,X,L+H)=-1。该配对 snc 且所有边界系数为一，lc 性给出下界负一。|Subtract the total transforms|The coefficient is one minus one minus one. Since the snc pair is lc, minus one is also the lower bound.
...
`);
// A chapter is one complete seminar session, with its own title and thanks board.
export const kmOutline=[];
for(const chapter of KM_CHAPTERS){
 chapter.start=kmOutline.length;
 kmOutline.push({kind:'cover',chapter:chapter.id,source:'Kollár–Mori (1998)',title:'第 '+chapter.id+' 章 · '+chapter.title,author:'János Kollár · Shigefumi Mori',tex:'',text:'Birational Geometry of Algebraic Varieties\n研读讨论班 · 定义、计算、定理与证明路线',en:{source:'Kollár–Mori (1998)',title:'Chapter '+chapter.id+' · '+chapter.en,author:'János Kollár · Shigefumi Mori',text:'Birational Geometry of Algebraic Varieties\nReading seminar · statements, calculations and proof maps'}});
 for(const part of KM_SECTIONS.filter(s=>s.chapter===chapter.id)){
  part.start=kmOutline.length;kmOutline.push(...kmBoards.filter(p=>p.section===part.id));part.end=kmOutline.length-1;
 }
 kmOutline.push({kind:'closing',chapter:chapter.id,source:'本章讨论结束',title:'谢谢！',author:'',tex:'',text:'',en:{source:'END OF CHAPTER',title:'Thank you!',author:'',text:''}});
