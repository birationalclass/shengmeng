...
export function createWeatherSky({panorama=true,renderer,device={},probe=false}={}){
 ...
 precision highp float;varying vec3 ray;uniform vec3 sunPosition,sunColor,seaColor;uniform float starsEnabled;uniform float seaHorizon;uniform float day,warm,direct,cloud,storm,clock,radius,showSun,stars,sidereal,twinkleTime;
 ...
 if(stars*starsEnabled>.0001)color+=(nightStars(d)+meteor(d))*stars*smoothstep(0.0,.18,d.y)*(1.-storm*.96);
