...
export function createWeatherSky({panorama=true}={}){
 ...
 const uniforms={sunPosition:{value:new THREE.Vector3(1,.5,0)},sunColor:{value:new THREE.Color('#fff4df')},day:{value:1},warm:{value:0},direct:{value:1},cloud:{value:.12},storm:{value:0},clock:{value:0},radius:{value:.00465},showSun:{value:1},stars:{value:0},sidereal:{value:0},galaxyMap:{value:fallback},galaxyMix:{value:0}};
 ...
 precision highp float;varying vec3 ray;uniform vec3 sunPosition,sunColor;uniform float day,warm,direct,cloud,storm,clock,radius,showSun,stars,sidereal;
 ...
 const mesh=new THREE.Mesh(new THREE.SphereGeometry(1,48,24),material);mesh.onBeforeRender=()=>{if(loadedAt)uniforms.galaxyMix.value=1-Math.exp(-(performance.now()-loadedAt)/1200);};mesh.name='Continuous Shanghai sky';mesh.frustumCulled=false;mesh.scale.setScalar(10000);return mesh;
}
