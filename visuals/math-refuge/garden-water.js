...
import {riverPoint,BUILDING_SCALE} from './site-layout.js?v=7-garden';
...
export function createGardenWater(scene){
  ...
  for(let i=0;i<=count;i++){
    ...
    for(let j=0;j<=across;j++){
      ...
      foam.push(Math.exp(-Math.pow((t-2/13)*55,2)));
  ...
  for(let i=0;i<80;i++){const a=i*2.399,r=.4+(i%13)/13*2.5;spray.push(-33+Math.cos(a)*r,3.1+(i%7)*.18,-40.6+Math.sin(a)*r);}
  ...
  for(let i=0;i<7;i++){matrix.makeTranslation(-6,-2.15,24.7+i*.9);bridge.setMatrixAt(i,matrix);}bridge.scale.setScalar(BUILDING_SCALE);bridge.receiveShadow=true;scene.add(bridge);
