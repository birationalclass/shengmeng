...
export function authoredContext(ctx){
 ...
 function text(value,x,y){for(const char of String(value)){const g=authoredGlyph(char),k=scale(),color=ctx.fillStyle;if(g){const width=Math.max(1.35,size()*.046);for(const path of g.paths){const points=path.map(([a,b])=>[x+a*k,y+(b-72)*k]);stroke(ctx,points,width,color);operations.push({rect:pointBox(points,width/2+1),strokePath:points,strokeWidth:width,chalkColor:color});}x+=g.advance*k;}else{const m=ctx.measureText(char);ctx.fillText(char,x,y);if(char.trim())operations.push({rect:[x-(m.actualBoundingBoxLeft||0)-1,y-(m.actualBoundingBoxAscent||size()*.8)-1,(m.actualBoundingBoxLeft||0)+(m.actualBoundingBoxRight??m.width)+2,(m.actualBoundingBoxAscent||size()*.8)+(m.actualBoundingBoxDescent||0)+2],chineseSpans:/[\u3400-\u9fff]/.test(char)?[[x,x+m.width]]:[]});x+=m.width;}}}
...
export async function authoredFormula(url){
 ...
 for(const element of [...root.querySelectorAll('path,rect,text,line,polyline,polygon,circle,ellipse')]){
  ...
  const record=(points,width)=>{for(const part of clipPolyline(points,clip))operations.push({rect:clipRect(pointBox(part,width/2+1),clip),strokePath:part,strokeWidth:width,chalkColor:'#eee9d5'});};
  ...
  if(glyph&&box.width&&box.height){
   ...
   const rect=clipRect(pointBox([[box.x,box.y],[box.x+box.width,box.y],[box.x,box.y+box.height],[box.x+box.width,box.y+box.height]].map(map),1),clip);if(rect)operations.push({rect});
  }
 }
