export const WEATHER_URL='https://api.open-meteo.com/v1/forecast?latitude=31.2304&longitude=121.4737&current=temperature_2m,relative_humidity_2m,precipitation,weather_code,cloud_cover,wind_speed_10m&daily=sunrise,sunset&timezone=Asia%2FShanghai&forecast_days=1';
...
export function parseWeather(data,now=Date.now()){
 ...
 return {fetched:now,time:c.time,code:c.weather_code,label:labels(c.weather_code),temperature:c.temperature_2m,cloud:Math.max(0,Math.min(1,c.cloud_cover/100)),rain:Math.max(0,c.precipitation||0),fog:c.weather_code===45||c.weather_code===48?1:0,wind:Math.max(0,c.wind_speed_10m||0),sunrise:data.daily?.sunrise?.[0]?.slice(11,16)||'',sunset:data.daily?.sunset?.[0]?.slice(11,16)||''};
}
export function createShanghaiWeather({onChange,fetcher=fetch,storage=globalThis.localStorage,now=Date.now}={}){
 let value=null,timer=null,stopped=false,controller=null;const key='refuge-shanghai-weather-v1';
