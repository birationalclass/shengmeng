import {reefMask,reefGLSL} from './shallow-reefs.js?v98-reef';
...
export function baseSeaDepthAt(x,z){
 ...
 const base=shallows+(88-shallows)*smooth(22,205,distance);const depth=base+(beachDepth(x,z)-base)*beachMask(x,z);return depth;
}
export function reefCoverage(x,z){return reefMask(x,z)*smooth(1.4,2.2,baseSeaDepthAt(x,z));}
export function seaDepthAt(x,z){return baseSeaDepthAt(x,z)-.45*reefCoverage(x,z);}
...
float baseSeaDepthAt(vec2 p){
 ...
 float depth=mix(mix(shallows,88.0,smoothstep(22.0,205.0,distance)),beachDepth(p),beachMask(p));return depth;
}`;
