...
for(const section of sections){
  ...
  const viewBox=[0,0,totalWidth,totalHeight],scale=Math.min(.050,1320/totalWidth,285/totalHeight);
  ...
  const standalone=`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="${totalWidth/1000*24}" height="${totalHeight/1000*24}" viewBox="${viewBox.join(' ')}">${inner}</svg>`;
  await fs.writeFile(new URL(formulaAsset,output),standalone);
  const svg=standalone.replace(/<svg[^>]*>/,`<svg x="${x}" y="${y}" width="${w}" height="${h}" viewBox="${viewBox.join(' ')}">`);
  const notes=chunks(section.text||'',33);
  const page=pages.length+1,asset=`page-${String(page).padStart(3,'0')}.svg`;
  const lines=notes.slice(0,3).map((line,i)=>`<text x="88" y="${465+i*42}" font-size="36">${chalkSVG(line)}</text>`).join('');
  ...
  pages.push({...section,asset:`./assets/chalk/${asset}`,formulaAsset:`./assets/chalk/${formulaAsset}`,formulaEm:viewBox[2]/1000,formulaRows,rows:[[80,22,1380,66],[80,95,0,0],[x-8,y-8,w+16,h+16],...notes.slice(0,3).map((_,i)=>[80,429+i*42,1380,42])]});
}
