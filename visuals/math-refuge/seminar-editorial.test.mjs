...
test('all 124 bilingual boards have complete prose and deterministic, bounded annotations',async()=>{
  ...
  for(const folder of ['','hu/','ye/','duan/']){
    ...
    for(const [index,page] of pages.entries()){
      ...
      if(report==='hu'&&index<16)assert(!page.annotation,'No forced notes on the opening hypotheses or results');
      if(report==='meng'&&index<5)assert(!page.annotation,'No redundant directions or diagonal-sum commentary');
  ...
  assert.equal(count,124);assert.equal(marked,16);assert.equal(notes,2);
});
