...
import {installBoards,paintBoard,cellPoint} from './board-world.mjs?v=20260923-sand-links1';
import {PLACES,THEMES,ease,clamp,cameraSpline,stageTime,arrivalPhase} from './journey.mjs?v=20260923-sand-links1';
...
export class SudokuAtlas extends AtlasScene{
 ...
 arrive(index,onReady,reduced=false){
  ...
  if(!this.built.has(index)&&!existing){this.constructions.set(index,{elapsed:0,seconds:decorationSeconds});this.assemble(index,0,decorationSeconds);}
  if(reduced){this.updateConstruction(0,true);this.updateSequence(duration,true);}
 }
 updateConstruction(dt,reduced=false){
  for(const [index,job]of this.constructions){
   const duration=3+BOARD_RISE_SECONDS+.15+job.seconds;
   job.elapsed=reduced?duration:Math.min(duration,job.elapsed+dt);
   this.assemble(index,Math.max(0,job.elapsed-3),job.seconds);
   if(job.elapsed>=duration){this.showBuilt(index);this.constructions.delete(index);}
  }
 }
