import {mobileDevice,mobileFrameRate,bindPinch} from './mobile.mjs?v=clarity4';
import {createCompletionRecords} from './records.mjs?v=clarity4';
...
let playerStorage=storage,campaign=new Campaign(model,storage);let decorationSeconds=readBuildSeconds(storage);
let selected=campaign.current-2,world,game,playing=false,busy=false,boardState,last=0,time=0,toastTimer,failed=false;
...
const completionRecords=createCompletionRecords({getCampaign:()=>campaign,t,storage,onOpen:()=>cameraKeys.clear(),onLogin:({account,boards,local})=>{
 const key=STORAGE_KEY+':'+account.id;
 playerStorage={getItem:k=>{try{return storage?.getItem(k===STORAGE_KEY?key:k);}catch{return null;}},setItem:(k,v)=>{try{storage?.setItem(k===STORAGE_KEY?key:k,v);}catch{}},removeItem:k=>storage?.removeItem(k===STORAGE_KEY?key:k)};
 // Old anonymous progress is carried forward only into a guest identity.
 try{if(account.kind==='guest'&&!playerStorage.getItem(STORAGE_KEY)&&storage?.getItem(STORAGE_KEY))playerStorage.setItem(STORAGE_KEY,storage.getItem(STORAGE_KEY));}catch{}
 if(local){campaign=new Campaign(model,playerStorage);selected=campaign.current-2;}else{campaign.storage=playerStorage;campaign.mergeCompleted(boards);if(world){world.restore(campaign.completed,{background:true});game?.updateCompleted?.(campaign.completed);sync();}}
}});
...
function sync(){
 ...
 $('overview').setAttribute('aria-label',t('全图','Atlas'));$('overview').title=t('全图','Atlas');$('language').textContent=en()?'中文':'EN';$('settingsToggle').setAttribute('aria-label',t('设置','Settings'));$('settingsToggle').title=t('设置','Settings');$('cameraHelp').textContent=mobileMode?t('单指拖动 · 双指轻缓缩放','Drag with one finger · Gentle pinch zoom'):t('↑ N · ↓ S · ← W · → E · 滚轮缩放 · 拖动环绕 · Shift＋方向键选格','↑ N · ↓ S · ← W · → E · Wheel to zoom · Drag to orbit · Shift + arrows select cells');syncFullscreen();
 $('clearLocalData').textContent=t('清除本地记录（Cookie）','Clear local data');$('clearDataNote').textContent=t('清除本机棋局与地图设置；已同步的通关进度会在登录后恢复。','Clear local boards and map settings. Synced progress is restored when you sign in.');
 $('clearDataTitle').textContent=t('清除本地记录？','Clear local data?');$('clearDataWarning').textContent=t('本机棋局与地图设置将被删除。服务器记录保留，已同步的通关进度将在登录后恢复；尚未完成的棋局无法恢复。','Local boards and map settings will be deleted. Server records remain and synced progress is restored on login. Unfinished boards cannot be recovered.');$('cancelClearData').textContent=t('取消','Cancel');$('confirmClearData').textContent=t('确认清除','Clear data');
...
function renderBoard(state){
 if(en()){for(const [action,label]of [['check','check'],['erase','clear']]){const button=$('gameMount').querySelector(`[data-action="${action}"]`);if(button)button.textContent=label;}}
 const status=$('gameMount').querySelector('.sudoku-status');if(status&&/^(数字键填数|Number keys to enter)/.test(status.textContent))status.textContent=mobileMode?t('单指拖动 · 双指轻缓缩放 · 点选格子后在下方填数','Drag · Gentle pinch zoom · Select a cell and tap a number below'):t('方向键 N/S/W/E 平移 · 滚轮缩放 · Shift＋方向键选格 · 数字键填数','Arrows move N/S/W/E · Wheel to zoom · Shift + arrows select cells · Number keys enter');
...
function openGame(index,{replay=false}={}){
 ...
 if(world){phase('assembly');world.arrive(index,()=>{phase('');sync();},false);}else phase('');
}
function finish(n){
 ...
 if(world&&n<9)world.cross(n-2,()=>{phase('');openGame(next);},false);else{phase('');openGame(next);}
}
...
function previewArchitecture(index){
 ...
 world.resetArchitecture(index);phase('assembly');world.arrive(index,()=>{phase('');document.body.dataset.phase='preview';$('stagePhase').textContent=world.constructions.has(index)?t('建筑正在搭建','BUILDING THE DOMAIN'):t('建筑预览 · 不影响通关进度','ARCHITECTURE PREVIEW · PROGRESS UNCHANGED');},false);
}
...
document.addEventListener('pointerdown',e=>{if(!$('settingsPanel').hidden&&!e.target.closest('#settingsPanel,#settingsToggle')){$('settingsPanel').hidden=true;$('settingsToggle').setAttribute('aria-expanded','false');}},true);
$('clearLocalData').onclick=()=>{$('clearDataDialog').showModal();$('cancelClearData').focus();};
$('cancelClearData').onclick=()=>$('clearDataDialog').close();
$('confirmClearData').onclick=()=>{try{clearLocalData(playerStorage);window.location.reload();}catch{$('clearDataDialog').close();notify(t('无法清除本地记录，请检查浏览器的存储权限。','Unable to clear local data. Please check browser storage permissions.'));}};
...
function syncFullscreen(){const active=!!fullscreenElement();if(world)world.fullscreen=active;document.documentElement.classList.toggle('is-fullscreen',active);$('fullscreen').setAttribute('aria-pressed',String(active));$('fullscreen').setAttribute('aria-label',active?t('退出全屏','Exit fullscreen'):t('全屏','Fullscreen'));}
...
let pointer=null;
canvas.addEventListener('pointerdown',e=>{if(e.button!==0||!world||busy||pinch.active)return;world.touring=false;pointer={id:e.pointerId,x:e.clientX,y:e.clientY,startX:e.clientX,startY:e.clientY,moved:false};canvas.setPointerCapture(e.pointerId);});
canvas.addEventListener('pointermove',e=>{if(!pointer||pointer.id!==e.pointerId||!world||pinch.active)return;const dx=e.clientX-pointer.x,dy=e.clientY-pointer.y;if(Math.hypot(e.clientX-pointer.startX,e.clientY-pointer.startY)>4)pointer.moved=true;if(pointer.moved)world.orbit(dx,dy);pointer.x=e.clientX;pointer.y=e.clientY;});
...
const pinch=bindPinch($('atlas'),{enabled:()=>mobileMode&&!!world&&!busy&&$('settingsPanel').hidden,zoom:ratio=>world?.pinchZoom(ratio),onStart:()=>{pointer=null;world.touring=false;lastInteraction=performance.now();},onEnd:()=>{pointer=null;lastInteraction=performance.now();}});
...
let layoutFrame=0;function scheduleLayout(){cancelAnimationFrame(layoutFrame);layoutFrame=requestAnimationFrame(()=>{world?.resize();if(world?.sequence?.kind==='assembly'){const pose=world.playPose(selected);world.sequence.keys[world.sequence.keys.length-1]=[2.8,pose.pos.toArray(),pose.aim.toArray()];}else if(world&&!busy&&!world.touring)world.focus(playing||document.body.classList.contains('previewing')?selected:-1);positionTargets();});}
...
document.addEventListener('fullscreenchange',onFullscreenChange);document.addEventListener('webkitfullscreenchange',onFullscreenChange);window.addEventListener('resize',scheduleLayout);window.visualViewport?.addEventListener('resize',scheduleLayout);document.addEventListener('visibilitychange',()=>{last=0;clearTimeout(animationTimer);cancelAnimationFrame(animationFrame);if(!document.hidden&&world&&!failed)animate(performance.now());});
let animationTimer=0,animationFrame=0,lastInteraction=0;
document.addEventListener('pointerdown',()=>lastInteraction=performance.now(),{passive:true});
function animate(now){if(document.hidden||failed){last=0;return;}const frameStart=performance.now();const dt=last?Math.min(.08,(now-last)/1000):0;last=now;time+=dt;if(!busy&&cameraKeys.size){world?.pan((cameraKeys.has('ArrowRight')?1:0)-(cameraKeys.has('ArrowLeft')?1:0),(cameraKeys.has('ArrowUp')?1:0)-(cameraKeys.has('ArrowDown')?1:0),dt);}if(world)world.musicLevels=music.readLevels(dt);world?.updateWorld(time,dt,false);if(busy&&world?.sequence?.kind==='assembly'){$('stagePhase').textContent=({moving:t('平稳抵达','ARRIVING'),settled:t('镜头已就位','CAMERA SETTLED'),building:t('建筑正在搭建','BUILDING THE DOMAIN'),board:t('棋盘展开','REVEALING THE BOARD')})[canvas.dataset.arrival]||'';}else if(world&&!busy&&(playing||document.body.classList.contains('previewing'))){$('stagePhase').textContent=world.constructions.has(selected)?(playing?t('棋盘已就绪 · 可以填数 · 建筑继续搭建','BOARD READY · ENTER NUMBERS · SCENERY BUILDING'):t('建筑正在搭建','BUILDING THE DOMAIN')):playing?'':t('建筑预览 · 不影响通关进度','ARCHITECTURE PREVIEW · PROGRESS UNCHANGED');}positionTargets();
 if(mobileMode){const fps=mobileFrameRate({moving:busy||!!world?.touring||!!pointer||pinch.active||!!cameraKeys.size,recent:now-lastInteraction<1500});canvas.dataset.renderFps=String(fps);animationTimer=setTimeout(()=>animationFrame=requestAnimationFrame(animate),Math.max(0,1000/fps-(performance.now()-frameStart)));}else animationFrame=requestAnimationFrame(animate);
}
...
await completionRecords.start();
try{await Promise.race([document.fonts.ready,new Promise(resolve=>setTimeout(resolve,1200))]);world=new SudokuAtlas(canvas,{mobile:mobileMode});world.decorationSeconds=decorationSeconds;REGIONS.forEach((r,i)=>world.paint(i,{values:campaign.board(r.n),selected:-1,cells:model.initial(r.n).map(v=>({classes:v?'given':''}))}));world.restore(campaign.completed);world.updateWorld(0,0,true);$('loading').classList.add('done');setTimeout(()=>$('loading').hidden=true,750);const preview=Number(new URLSearchParams(location.search).get('preview'));if(Number.isInteger(preview)&&preview>=2&&preview<=9)previewArchitecture(preview-2);else if(campaign.completed.length===8)showAtlas(true);else openGame(campaign.current-2);animate(performance.now());}catch(error){fallback(error);}
