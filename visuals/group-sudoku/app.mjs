import {mobileDevice,mobileFrameRate,bindPinch} from './mobile.mjs?v=mobile1';
import {createCompletionRecords} from './records.mjs?v=background1';
...
import {Campaign,STORAGE_KEY,MAP_STYLE_KEY,clearLocalData} from './campaign.mjs?v=background1';
import {SudokuAtlas,REGIONS} from './atlas.mjs?v=mobile1';
...
let selected=campaign.current-2,world,game,playing=false,busy=false,boardState,last=0,time=0,toastTimer,mapStyle='parchment',failed=false;
...
function sync(){
 ...
 $('overview').textContent=t('全图','Atlas');$('language').textContent=en()?'中文':'EN';$('settingsToggle').textContent=t('设置','Settings');$('cameraHelp').textContent=mobileMode?t('单指拖动 · 双指轻缓缩放','Drag with one finger · Gentle pinch zoom'):t('↑ N · ↓ S · ← W · → E · 滚轮缩放 · 拖动环绕 · Shift＋方向键选格','↑ N · ↓ S · ← W · → E · Wheel to zoom · Drag to orbit · Shift + arrows select cells');syncFullscreen();
...
let animationTimer=0,animationFrame=0,lastInteraction=0,frameCost=0,rendered=0;
...
try{await Promise.race([document.fonts.ready,new Promise(resolve=>setTimeout(resolve,1200))]);world=new SudokuAtlas(canvas,{mobile:mobileMode});world.decorationSeconds=decorationSeconds;world.setMapStyle(mapStyle);REGIONS.forEach((r,i)=>world.paint(i,{values:campaign.board(r.n),selected:-1,cells:model.initial(r.n).map(v=>({classes:v?'given':''}))}));world.restore(campaign.completed);world.updateWorld(0,0,true);$('loading').classList.add('done');setTimeout(()=>$('loading').hidden=true,750);const preview=Number(new URLSearchParams(location.search).get('preview'));if(Number.isInteger(preview)&&preview>=2&&preview<=9)previewArchitecture(preview-2);else if(campaign.completed.length===8)showAtlas(true);else openGame(campaign.current-2);animate(performance.now());}catch(error){fallback(error);}
