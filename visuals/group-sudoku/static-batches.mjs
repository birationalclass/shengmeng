...
export function batchBuiltDomain(atlas,board){
 ...
 const dynamic=new Set([board.lift,board.glow,...(atlas.iceSkaters||[]).map(s=>s.root),...atlas.rotating.map(t=>t.object),...(atlas.detailMotion||[]).map(t=>t.object),...(atlas.clockworkLifts||[])]),groups=new Map();
