...
export function batchBuiltDomain(atlas,board){
 ...
 const dynamic=new Set([board.lift,board.glow,...(atlas.poolBoats||[]).map(b=>b.root),...(atlas.owls||[]).map(o=>o.root),...(atlas.iceSkaters||[]).map(s=>s.root),...atlas.rotating.map(t=>t.object),...(atlas.detailMotion||[]).map(t=>t.object),...(atlas.clockworkLifts||[])]),groups=new Map();
