...
export function landmark(a,parent,type,x,z,scale=1){
 ...
 if(type==='well'||type==='fountain'){
  ...
  if(type==='fountain'){cyl(.17,1.1,[0,.93,0],M.paper);cyl(.47,.11,[0,1.43,0],M.brass);a.mesh(g,new T.SphereGeometry(.14,12,8),M.gold,[0,1.72,0]);for(let k=0;k<6;k++){const q=k*Math.PI/3;const curve=new T.QuadraticBezierCurve3(new T.Vector3(0,1.62,0),new T.Vector3(Math.sin(q)*.7,1.5,Math.cos(q)*.7),new T.Vector3(Math.sin(q)*.66,.45,Math.cos(q)*.66));a.mesh(g,new T.TubeGeometry(curve,10,.013,4,false),M.paper);}}
  else{for(const sign of [-1,1])box([.12,1.8,.15],[sign*.77,1.27,0],M.wood);rod([-.91,1.75,0],[.91,1.75,0],.07,M.wood);rod([0,1.75,0],[0,.65,0],.023,M.dark);for(const sign of [-1,1]){const r=box([1.1,.1,1.6],[sign*.44,2.26,0],M.dark);r.rotation.z=-sign*.48;}cyl(.20,.27,[0,.57,0],M.wood);}
 }
 if(type==='pavilion'){
 ...
 if(type==='observatory'){
  ...
  cyl(1.17,.13,[0,1.82,0],M.brass);a.mesh(g,new T.SphereGeometry(1.08,24,12,0,Math.PI*2,0,Math.PI/2),M.jade,[0,1.88,0]);for(const sign of [-1,1]){const pts=Array.from({length:17},(_,k)=>new T.Vector3(sign*.17,1.88+Math.sin(k*Math.PI/16)*1.08,Math.cos(k*Math.PI/16)*1.06));a.mesh(g,new T.TubeGeometry(new T.CatmullRomCurve3(pts),16,.035,5,false),M.brass);}rod([0,2.03,0],[0,2.76,1.20],.10,M.dark);rod([0,2.74,1.16],[0,2.88,1.4],.14,M.brass);
 }
 if(type==='obelisk'){
