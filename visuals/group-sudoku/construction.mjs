...
export function constructionTiming(part,seconds=15){if(part.board)return {delay:0,duration:BOARD_RISE_SECONDS};if(!part.decoration)return {delay:part.delay,duration:part.duration};const longest=Math.min(seconds*.3,2.6),duration=part.twoStep||part.order===part.count-1?longest:Math.min(longest,1.05);return {delay:BOARD_RISE_SECONDS+.15+part.order/Math.max(1,part.count-1)*(seconds-longest),duration};}
