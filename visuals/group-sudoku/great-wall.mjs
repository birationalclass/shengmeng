...
export function greatWall(a,parent){
 const wall=new T.Group();wall.name='Great Wall · 万里长城';wall.userData.landmark='great-wall';parent.add(wall);
 ...
 const at=q=>{const radius=10.25+.55*Math.cos(q*5.4);return new T.Vector3(Math.sin(q)*radius,0,-Math.cos(q)*radius);};
 // A rear arc leaves the complete board and its labels unobstructed.
 for(let i=0;i<26;i++){
  const q0=-1.10+i*2.20/26,q1=-1.10+(i+1)*2.20/26,p=at(q0),r=at(q1),d=r.clone().sub(p),len=d.length()+.045;
  const h=1.35+.80*(1+Math.cos((q0+q1)*3.2))/2;
  const segment=new T.Group();segment.position.copy(p.add(r).multiplyScalar(.5));segment.rotation.y=Math.atan2(d.x,d.z);wall.add(segment);
 ...
 for(const [j,q] of [-.88,0,.88].entries()){
  ...
  a.mesh(flame,new T.ConeGeometry(.10,.34,5),a.materials.gold,[0,.30,0]);
