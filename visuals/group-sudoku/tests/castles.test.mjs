...
test('wizard court has opposing armies of all six piece types; mechanical scene has 47 exposed gears',()=>{const {p}=build(3);const pieces=[];p.traverse(o=>{if(o.userData.army)pieces.push(o);});assert.equal(pieces.length,16);assert.equal(new Set(pieces.map(o=>o.userData.landmark)).size,6);const {a}=build(4);assert.equal(a.rotating.filter(o=>o.object.userData.gearTrain).length,47);});
