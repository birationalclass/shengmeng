...
export function setupLayoutPreview({scene,retreat,camera,controls,rooms,stop,setHour,onMove}){
 ...
 const bar=document.createElement('aside');bar.id='layoutPreviewBar';bar.innerHTML='<strong>难民营设计 · 10 米网格</strong><span>建筑 · 装饰 · 平台连桥</span><a href="?plan=bridges-20260926">查看 9月26日布局 · 连桥方案</a><div><button data-view="all">总览</button><button data-view="01B">01B 报告厅</button><button data-view="west">报告厅看日落</button><button data-view="01C">01C 固定备份</button><button data-view="north">北侧群落</button><button data-view="south">南侧群落</button></div><label><input id="layoutGrid" type="checkbox" checked>10m 网格 / 观景边界</label><label><input id="layoutLabels" type="checkbox" checked>建筑编号</label><a href="../math-refuge/?qa=arrival137" target="_blank" rel="noopener">打开原版对照</a><small>左键观察 · 右键移动建筑 · 滚轮前后 · W/S 前后，A/D 转向，Q/E 平移，空格/X 升降</small>';
