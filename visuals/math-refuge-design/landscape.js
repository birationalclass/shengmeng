import {decorationGroup} from './design-groups.js';
...
export async function createLandscape(renderer,scene,report){
  ...
  function add(geometry,material,x,y,z,scale,angle,name,shadow=true){
    ...
    if(!batches.has(key))batches.set(key,{geometry,material,matrices:[],groups:[],name,shadow});
    dummy.position.set(x,y,z).multiplyScalar(BUILDING_SCALE);dummy.scale.set(...(Array.isArray(scale)?scale:[scale,scale,scale]));dummy.rotation.set(0,angle,0);dummy.updateMatrix();batches.get(key).matrices.push(dummy.matrix.clone());batches.get(key).groups.push(decorationGroup()||'plant-'+x+','+y+','+z);
  }
  ...
  function finish(){
    for(const {geometry,material,matrices,groups,name,shadow} of batches.values()){
      const mesh=new THREE.InstancedMesh(geometry,material,matrices.length);mesh.name=name;mesh.userData.designGroups=groups;matrices.forEach((m,i)=>mesh.setMatrixAt(i,m));mesh.castShadow=shadow;mesh.receiveShadow=true;mesh.computeBoundingSphere();mesh.boundingSphere.radius+=.35;scene.add(mesh);
    }
  }
