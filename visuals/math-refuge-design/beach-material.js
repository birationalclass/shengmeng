...
export function createBeachMaterial(seaLevel,study=null){
 const clock={value:0},material=new THREE.MeshStandardMaterial({color:'#c9b88f',roughness:.94});
 material.onBeforeCompile=shader=>{
 if(study){shader.uniforms.studyFoam=study.foam;shader.uniforms.studyEnabled=study.enabled;}
 ...
 shader.fragmentShader=(study?'uniform sampler2D studyFoam;uniform float studyEnabled;\n':'')+`varying vec3 sandWorld;uniform float sandTime,sandSea;
 ...
 ${study?`vec2 studyUV=(vec2(88.-sandWorld.x,sandWorld.z+35.)-vec2(-35.,-45.))/vec2(60.,160.);
 if(studyEnabled>.5&&all(greaterThan(studyUV,vec2(0.)))&&all(lessThan(studyUV,vec2(1.))))damp=max(damp,texture2D(studyFoam,studyUV).g);`:''}
