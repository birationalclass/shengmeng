import {setupLayoutPreview,layout} from './layout-preview.js';
import {openingArrivalProfile,createOpeningRoute,APPROACH_SPEED,DOOR_SPEED,BRAKE_SECONDS} from './opening-arrival.js?v141';
...
import {createRetreat} from './scene.js?v142';
import {createLecture} from './lecture.js?v137';
...
function enterScene(){
  ...
  opening=null;touring=false;free=true;blend=null;controls.enabled=true;
...
function replayOpening(){
  ...
  sunriseIntro.prepare(solarEvents(sceneTime.date).sunrise);retreat?.fleet.resetSunrise();$('timeRate').value='1';
...
function selectShot(index,reportView=false,openingArrival=false){
  ...
  beginTransition();blend.openingArrival=openingArrival;
  if(openingArrival){
    const gateX=HALL.west*BUILDING_SCALE,eye=DECK_Y*BUILDING_SCALE+1.7;
    const brakeDistance=(APPROACH_SPEED+DOOR_SPEED)*BRAKE_SECONDS/2;
    const doorway=new THREE.Vector3(gateX-brakeDistance,eye,0);
    const destination=new THREE.Vector3(37.7*BUILDING_SCALE,eye,0);
    const route=createOpeningRoute(THREE,blend.position,doorway);
    const farDistance=route.getLength(),arrival=openingArrivalProfile(farDistance,destination.x-gateX);
    Object.assign(blend,{arrival,farDistance,doorway,endPosition:destination,endDistance:10,
      endRotation:new THREE.Quaternion().setFromRotationMatrix(lookMatrix.lookAt(destination,destination.clone().add(new THREE.Vector3(10,0,0)),viewUp)),
      route,duration:arrival.duration});
  }
  updateLabels();
...
function openingArrivalPosition(move,elapsed,out){
  const state=move.arrival.sample(elapsed);
  if(state.distance<move.farDistance)move.route.getPointAt(Math.max(0,state.distance/move.farDistance),out);
  else out.copy(move.doorway).add(new THREE.Vector3(state.distance-move.farDistance,0,0));
  return out;
}
function applyShot(dt){
  if(blend?.arrival){
    blend.elapsed+=dt;const state=blend.arrival.sample(blend.elapsed);
    openingArrivalPosition(blend,blend.elapsed,camera.position);
    const rotationProgress=smoothProgress(Math.min(1,state.distance/blend.farDistance));
    const tangent=state.distance<blend.farDistance?blend.route.getTangentAt(Math.max(0,state.distance/blend.farDistance)):new THREE.Vector3(1,0,0);
    const facing=new THREE.Quaternion().setFromRotationMatrix(lookMatrix.lookAt(camera.position,camera.position.clone().add(tangent),viewUp));
    camera.quaternion.slerpQuaternions(blend.rotation,facing,smoothProgress(Math.min(1,blend.elapsed/2.5)));
    viewDirection.set(0,0,-1).applyQuaternion(camera.quaternion);controls.target.copy(camera.position).addScaledVector(viewDirection,10);
    camera.fov=THREE.MathUtils.lerp(blend.fov,SHOTS[shot].fov,rotationProgress);camera.updateProjectionMatrix();controls.update();
    $('world').dataset.arrivalPhase=state.phase;$('world').dataset.arrivalSpeed=state.speed.toFixed(3);
    if(blend.elapsed>=blend.arrival.doorTime&&!blend.sunriseStarted){sunriseIntro.arrive();retreat.fleet.startSunrise();blend.sunriseStarted=true;}
    if(state.phase==='complete'){blend=null;free=true;}
    return;
  }
...
function tick(stamp){
  ...
  // Sense the real camera and the upcoming curved flight, independently of hover.
  const doorPath=[camera.position];
  if(blend){
    for(let ahead=.25;ahead<=1.5;ahead+=.25){
      const t=Math.min(1,(blend.elapsed+ahead)/blend.duration),p=new THREE.Vector3();
      if(blend.arrival)openingArrivalPosition(blend,blend.elapsed+ahead,p);
      else if(blend.route)blend.route.getPointAt(smoothProgress(t),p);
      else for(const axis of ['x','y','z'])p[axis]=motionCoordinate(blend.position[axis],blend.endPosition[axis],blend.velocity[axis],blend.acceleration[axis],blend.duration,t);
      doorPath.push(p);if(t===1)break;
    }
  }
  retreat?.campus.automaticDoors.update(dt,reduced.matches,doorPath);
  ...
  const hallLayout=layout.find(b=>b[0]==='01B');
  const hallCenter=new THREE.Vector3(39*BUILDING_SCALE+hallLayout[4],3,-hallLayout[5]),nearHall=camera.position.distanceTo(hallCenter)<65;
...
try{
  ...
  setupLayoutPreview({scene,retreat,camera,controls,rooms,stop:()=>{opening=null;touring=false;free=true;blend=null;keys.clear();controls.enabled=true;sunriseIntro.cancel();$('mode').textContent='难民营设计 · 自由观察';},onMove:()=>{roomViews.forEach((v,i)=>{rooms[i].root.updateWorldMatrix(true,true);v.bounds=new THREE.Box3().setFromObject(rooms[i].root);v.consoleBounds=new THREE.Box3().setFromObject(roomLecterns[i].group);});renderer.shadowMap.needsUpdate=true;},setHour:hour=>{sunriseIntro.cancel();sceneTime.previewAt(hour==='sunset'?eventHours().sunset-.12:hour);}});
  roomViews.forEach((v,i)=>{rooms[i].root.updateWorldMatrix(true,true);v.bounds=new THREE.Box3().setFromObject(rooms[i].root);v.consoleBounds=new THREE.Box3().setFromObject(roomLecterns[i].group);});
  $('loading').dataset.entryRequested='true';
...
function installPhysicalControls(){
  ...
  bindPhysicalButtons($('world'),controls,hit,action=>{
    if(action.startsWith('screen:site:')){const site=VIDEO_SITES.find(s=>s.id===action.slice(12));if(site)screenWebview.open(site);}
