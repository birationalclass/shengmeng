import {decoration,decorationGroup} from './design-groups.js';
import {relocateArchitecture} from './layout-preview.js';
...
import {createBoats} from './boats.js?v142';
...
import {createCampus} from './campus.js?v136';
...
export async function createRetreat(renderer,scene,report,device={}){
  ...
  function instance(geo,material,p,s,r=[0,0,0]){
    ...
    if(!batches.has(key))batches.set(key,{geo,material,matrices:[],groups:[],region});
    dummy.position.fromArray(p);dummy.scale.fromArray(s);dummy.rotation.set(...r);dummy.updateMatrix();
    batches.get(key).matrices.push(dummy.matrix.clone());batches.get(key).groups.push(decorationGroup());
  }
  ...
  function buildPlatforms(){
    for(const y of new Set(layoutFloors.map(f=>f.y))){
      ...
      for(const [a,c,b,d] of union.edges){
        ...
        // Preview decks receive their own borders; omit old merged ground-edge strips.
      }
  ...
  function planterParts(x,y,z,w=2){
  ...
  function treeParts(x,y,z,scale=1){
    landscape.tree(x,y,z,scale*BUILDING_SCALE);
  }
  function sofaParts(x,y,z,angle=0){
  ...
  function tableParts(x,y,z,w=2,d=1){
  ...
  const planter=decoration('花箱',planterParts),tree=decoration('树木',treeParts),sofa=decoration('沙发',sofaParts),table=decoration('桌子',tableParts);
  ...
  const previewPlants=new Set(scene.children.filter(o=>!architectureObjects.has(o)));
  ...
  for(const {geo,material,matrices,groups,region} of batches.values()){
    const mesh=new THREE.InstancedMesh(geo,material,matrices.length);
    matrices.forEach((matrix,i)=>mesh.setMatrixAt(i,matrix));mesh.userData.designGroups=groups;
  ...
  relocateArchitecture(scene,previewPlants,true);
