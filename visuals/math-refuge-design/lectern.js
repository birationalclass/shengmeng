...
export function createLectern(T){
  ...
  function update({playing=false,page=0,total=0,seeking=false,retractable=false,stored=false,storageProgress=0}={}){
    if(!screenEnabled)return;
    storagePad.visible=retractable;
    const key=[playing,page,total,seeking,retractable,stored,Math.round(storageProgress*60),pads.map(p=>p.userData.hovered?1:0).join('')].join(':');if(last===key)return;last=key;
    ...
    if(retractable){ctx.fillStyle='#34544c';ctx.fillRect(674,85,310,84);drawPulley(ctx,830,127,70,storageProgress,stored);}
