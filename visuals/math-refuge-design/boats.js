...
export function createBoats(scene){
  ...
  // From the east-facing hall, negative Z is screen-left. Keep a 17 m
  // separation from the existing moored boat at the end of the crossing.
  add('sail',(HALL.west+HALL.east)/2+115/S,-30/S,Math.PI,4);
  const dawnBoat=boats.at(-1);dawnBoat.name='Sunrise crossing sailboat';
  let crossing=null;
  const resetSunrise=()=>{crossing=null;dawnBoat.position.z=-30;dawnBoat.userData.mooring[1]=-30/S;};

  [[77,65,.35],[-17,65,-.8]].forEach(([x,z,a],i)=>add('kayak',x,z,a,i));
  let time=0;
  return {root,boats,resetSunrise,startSunrise(){crossing=0;},update(dt,tide=seaLevel*S){
    dt=Math.max(0,Math.min(dt,.1));time+=dt;
    if(crossing!==null){crossing=Math.min(45,crossing+dt);const u=crossing/45;
      // Positive, gently varying drift speed; endpoints retain zero velocity and acceleration.
      const phase=u+.003*Math.sin(18*Math.PI*u)+.004*Math.sin(8*Math.PI*u),k=phase*phase*phase*(10+phase*(-15+6*phase));dawnBoat.position.z=-30+52*k;dawnBoat.userData.mooring[1]=dawnBoat.position.z/S;}
boats.forEach((boat,i)=>{boat.position.y=tide+.04+.055*Math.sin(time*.7+i*1.9);boat.rotation.z=.022*Math.sin(time*.6+i);boat.rotation.x=.012*Math.cos(time*.8+i);if(boat===dawnBoat){boat.position.y+=.085*Math.sin(time*1.35+.4)+.035*Math.sin(time*2.13);boat.rotation.z+=.017*Math.sin(time*1.35);boat.rotation.x+=.013*Math.sin(time*1.35+.8);}});},dispose(){geometries.forEach(g=>g.dispose());materials.forEach(m=>m.dispose());scene.remove(root);}};
}
