...
const views={
 pool:{position:[1.7,2.0,6.5],target:[-1.3,2.05,-6.0]},
 terrace:{position:[-3.0,2.7,3.8],target:[-4.2,1.65,-3.0]},
 overview:{position:[13,12,17],target:[0,1.1,-3]},
 back:{position:[13,9,-20],target:[0,2,-7]}
};
const error=message=>{
...
function setMode(value){
 ...
 if(value==='render'){$('loading').hidden=true;status.textContent='静态精修图 · 返回自由探索即可旋转';}
...
function quality(){
 ...
 if(model)model.traverse(o=>{if(o.name.startsWith('Garden_shrub'))o.visible=v!=='low';});
 resize();
}
...
async function init(){
 try{
  ...
  scene=new THREE.Scene();scene.background=new THREE.Color('#bdced5');scene.fog=new THREE.Fog('#c3d2d7',65,160);
  camera=new THREE.PerspectiveCamera(55,1,.08,250);
  controls=new OrbitControls(camera,canvas);controls.enableDamping=true;controls.dampingFactor=.075;controls.minDistance=2.5;controls.maxDistance=55;controls.minPolarAngle=.10;controls.maxPolarAngle=Math.PI*.49;controls.autoRotateSpeed=.32;controls.maxTargetRadius=28;controls.cursor.set(0,1,-3);
  ...
  sun=new THREE.DirectionalLight(0xffe0b2,2.5);sun.position.set(-10,16,10);sun.target.position.set(0,0,-3);sun.castShadow=true;
  Object.assign(sun.shadow.camera,{left:-20,right:20,top:20,bottom:-20,near:.5,far:65});sun.shadow.mapSize.set(2048,2048);sun.shadow.normalBias=.035;sun.shadow.bias=-.00008;scene.add(sun,sun.target);
  const sky=new Sky();sky.scale.setScalar(180);const u=sky.material.uniforms;u.turbidity.value=3;u.rayleigh.value=1.3;u.mieCoefficient.value=.004;u.mieDirectionalG.value=.8;u.sunPosition.value.copy(sun.position).normalize();
  const envScene=new THREE.Scene();envScene.add(sky);const pmrem=new THREE.PMREMGenerator(renderer);envTarget=pmrem.fromScene(envScene,.03,.1,200);scene.environment=envTarget.texture;pmrem.dispose();sky.geometry.dispose();sky.material.dispose();
  ...
  const gltf=await loader.loadAsync('./assets/courtyard.glb',ev=>{
 ...
 }catch(e){console.error(e);error('请检查网络与浏览器硬件加速，或先查看静态精修图。');}
}
...
canvas.addEventListener('webglcontextlost',e=>{e.preventDefault();error('图形资源已暂停，请重新加载，或选择静态精修图。');});
