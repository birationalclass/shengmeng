...
5. Set `RECORDS_SECRET` and `RECORDS_ADMIN_PASSWORD` using `wrangler secret put` and private input. Use a persistent random signing secret of at least 32 characters and a teacher password of at least 16 characters. Securely provide the password to the teacher; never embed it in the webpage.
